...
def from_image(img):
    ...
    TensorPlay-specific helper kept from the legacy tensorplay.vision.transforms
    module; uses the optimized C++ path when possible.
    """
    import numpy as _np

    if isinstance(img, Image.Image):
        if img.mode == 'I':
            arr = _np.array(img, _np.int32, copy=False)
        elif img.mode == 'I;16':
            arr = _np.array(img, _np.int16, copy=False)
        elif img.mode == 'F':
            arr = _np.array(img, _np.float32, copy=False)
        elif img.mode == '1':
            arr = 255 * _np.array(img, _np.uint8, copy=False)
        else:
            arr = _np.array(img, _np.uint8)
...
def from_file(path, backend=None):
    """Read an image from path and return a Tensor (legacy helper)."""
