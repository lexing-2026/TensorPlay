...
from ...ops.misc import Conv3dNormActivation
...
class SepInceptionBlock3D(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x2 = self.branch2(x)
        x3 = self.branch3(x)
        out = tensorplay.cat((x0, x1, x2, x3), 1)
        return out


class S3D(nn.Module):
    """Separable 3D CNN over (batch, channel, time, height, width) video clips.
...
def s3d(*, weights: Optional[S3D_Weights] = None, progress: bool = True, **kwargs: Any) -> S3D:
    """Separable 3D CNN model from
    `Rethinking Spatiotemporal Feature Learning <https://arxiv.org/abs/1712.04851>`__.

    Args:
        weights (:class:`~tensorplay.vision.models.video.S3D_Weights`, optional): The
            pretrained weights to use. See
            :class:`~tensorplay.vision.models.video.S3D_Weights`
            below for more details, and possible values. By default, no
            pre-trained weights are used.
        progress (bool): If True, displays a progress bar of the download to stderr. Default is True.
        **kwargs: parameters passed to the ``tensorplay.vision.models.video.s3d.S3D`` base class.
    """
