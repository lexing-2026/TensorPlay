...
def _unsqueeze(x: Tensor, target_dim: int, expand_dim: int) -> tuple[Tensor, int]:
    tensor_dim = x.dim()
...
def _squeeze(x: Tensor, target_dim: int, expand_dim: int, tensor_dim: int) -> Tensor:
    if tensor_dim == target_dim - 1:
...
class Pool(nn.Module):
    ...
    def forward(self, x: Tensor, thw: tuple[int, int, int]) -> tuple[Tensor, tuple[int, int, int]]:
        x, tensor_dim = _unsqueeze(x, 4, 1)
        # Separate the class token and reshape the input
        class_token, x = tensorplay.tensor_split(x, [1], dim=2)
        x = x.transpose(2, 3)
        B, N, C = x.shape[:3]
        ...
        x = tensorplay.cat((class_token, x), dim=2)
...
def _interpolate(embedding: Tensor, d: int) -> Tensor:
...
def _add_rel_pos(
    ...
    """Fold factored relative position embeddings into the attention logits."""
    q_t, q_h, q_w = q_thw
    k_t, k_h, k_w = k_thw
    ...
    dist_h = tensorplay.arange(q_h)[:, None] * q_h_ratio - (tensorplay.arange(k_h)[None, :] + (1.0 - k_h)) * k_h_ratio
    q_w_ratio = max(k_w / q_w, 1.0)
    k_w_ratio = max(q_w / k_w, 1.0)
    dist_w = tensorplay.arange(q_w)[:, None] * q_w_ratio - (tensorplay.arange(k_w)[None, :] + (1.0 - k_w)) * k_w_ratio
    q_t_ratio = max(k_t / q_t, 1.0)
    k_t_ratio = max(q_t / k_t, 1.0)
    dist_t = tensorplay.arange(q_t)[:, None] * q_t_ratio - (tensorplay.arange(k_t)[None, :] + (1.0 - k_t)) * k_t_ratio
    ...
    rel_h_q = tensorplay.einsum("bythwc,hkc->bythwk", r_q, Rh)  # [B, H, q_t, qh, qw, k_h]
    rel_w_q = tensorplay.einsum("bythwc,wkc->bythwk", r_q, Rw)  # [B, H, q_t, qh, qw, k_w]
    ...
    rel_q_t = tensorplay.matmul(r_q, Rt.transpose(1, 2)).transpose(0, 1)
...
def _add_shortcut(x: Tensor, shortcut: Tensor, residual_with_cls_embed: bool) -> Tensor:
...
class MultiscaleAttention(nn.Module):
    def __init__(
        ...
        self.rel_pos_t: Optional[Tensor] = None
        if rel_pos_embed:
            ...
            self.rel_pos_h = nn.Parameter(tensorplay.zeros(spatial_dim, self.head_dim))
            self.rel_pos_w = nn.Parameter(tensorplay.zeros(spatial_dim, self.head_dim))
            self.rel_pos_t = nn.Parameter(tensorplay.zeros(temporal_dim, self.head_dim))
    ...
    def forward(self, x: Tensor, thw: tuple[int, int, int]) -> tuple[Tensor, tuple[int, int, int]]:
        ...
        attn = tensorplay.matmul(self.scaler * q, k.transpose(2, 3))
...
class PositionalEncoding(nn.Module):
    def __init__(self, embed_size: int, spatial_size: tuple[int, int], temporal_size: int, rel_pos_embed: bool) -> None:
        ...
        self.class_token = nn.Parameter(tensorplay.zeros(embed_size))

        self.spatial_pos: Optional[Tensor] = None
        self.temporal_pos: Optional[Tensor] = None
        self.class_pos: Optional[Tensor] = None
        if not rel_pos_embed:
            self.spatial_pos = nn.Parameter(tensorplay.zeros(self.spatial_size[0] * self.spatial_size[1], embed_size))
            self.temporal_pos = nn.Parameter(tensorplay.zeros(self.temporal_size, embed_size))
            self.class_pos = nn.Parameter(tensorplay.zeros(embed_size))

    def forward(self, x: Tensor) -> Tensor:
        class_token = self.class_token.expand(x.size(0), -1).unsqueeze(1)
        x = tensorplay.cat((class_token, x), dim=1)
        ...
        if self.spatial_pos is not None and self.temporal_pos is not None and self.class_pos is not None:
            ...
            pos_embedding = tensorplay.repeat_interleave(self.temporal_pos, hw_size, dim=0)
            pos_embedding.add_(self.spatial_pos.unsqueeze(0).expand(self.temporal_size, -1, -1).reshape(-1, embed_size))
            pos_embedding = tensorplay.cat((self.class_pos.unsqueeze(0), pos_embedding), dim=0).unsqueeze(0)
...
class MViT_V1_B_Weights(WeightsEnum):
    KINETICS400_V1 = Weights(
        ...
        meta={
            ...
            "_docs": (
                "The accuracies are estimated on video-level "
...
def mvit_v1_b(*, weights: Optional[MViT_V1_B_Weights] = None, progress: bool = True, **kwargs: Any) -> MViT:
    """Base MViTV1 architecture from
    `Multiscale Vision Transformers <https://arxiv.org/abs/2104.11227>`__.

    Args:
        weights (:class:`~tensorplay.vision.models.video.MViT_V1_B_Weights`, optional): The
            pretrained weights to use. See
            :class:`~tensorplay.vision.models.video.MViT_V1_B_Weights`
            below for more details, and possible values. By default, no
            pre-trained weights are used.
        progress (bool, optional): If True, displays a progress bar of the
            download to stderr. Default is True.
        **kwargs: parameters passed to the ``tensorplay.vision.models.video.mvit.MViT``
            base class.
    """
...
def mvit_v2_s(*, weights: Optional[MViT_V2_S_Weights] = None, progress: bool = True, **kwargs: Any) -> MViT:
    """Small MViTV2 architecture from
    `Multiscale Vision Transformers <https://arxiv.org/abs/2104.11227>`__ and
    `MViTv2: Improved Multiscale Vision Transformers for Classification and Detection
    <https://arxiv.org/abs/2112.01526>`__.

    Args:
        weights (:class:`~tensorplay.vision.models.video.MViT_V2_S_Weights`, optional): The
            pretrained weights to use. See
            :class:`~tensorplay.vision.models.video.MViT_V2_S_Weights`
            below for more details, and possible values. By default, no
            pre-trained weights are used.
        progress (bool, optional): If True, displays a progress bar of the
            download to stderr. Default is True.
