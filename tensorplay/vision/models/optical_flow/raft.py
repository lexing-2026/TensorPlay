"""RAFT optical-flow models: recurrent all-pairs field transforms with a correlation pyramid."""
...
from tensorplay.nn import BatchNorm2d, InstanceNorm2d
...
class ResidualBlock(nn.Module):
    """Residual block with an extra trailing relu and biased convolutions."""

    def __init__(self, in_channels, out_channels, *, norm_layer, stride=1, always_project: bool = False):
        super().__init__()

        # Biases stay on even before norm layers: some released checkpoints froze their
        # norm statistics after early training, which keeps the bias term informative.
...
class BottleneckBlock(nn.Module):
    """Residual bottleneck block with an extra trailing relu and biased convolutions."""

    def __init__(self, in_channels, out_channels, *, norm_layer, stride=1):
        super().__init__()

        # See the bias note in ResidualBlock.
        self.convnormrelu1 = Conv2dNormActivation(
...
class FeatureEncoder(nn.Module):
    """Feature encoder used both for the per-frame features and for the context.
...
class MotionEncoder(nn.Module):
    """Motion encoder of the update block.
    ...
    def forward(self, flow, corr_features):
        ...
        corr_flow = tensorplay.cat([corr, flow], dim=1)
        corr_flow = self.conv(corr_flow)
        return tensorplay.cat([corr_flow, flow_orig], dim=1)
...
class ConvGRU(nn.Module):
    ...
    def forward(self, h, x):
        hx = tensorplay.cat([h, x], dim=1)
        z = tensorplay.sigmoid(self.convz(hx))
        r = tensorplay.sigmoid(self.convr(hx))
        q = tensorplay.tanh(self.convq(tensorplay.cat([r * h, x], dim=1)))
        h = (1 - z) * h + z * q
        return h
...
class RecurrentBlock(nn.Module):
    """Recurrent block of the update block.
...
class UpdateBlock(nn.Module):
    """The update block: motion encoder, recurrent block, and flow head.
    ...
    def forward(self, hidden_state, context, corr_features, flow):
        ...
        x = tensorplay.cat([context, motion_features], dim=1)
...
class MaskPredictor(nn.Module):
    """Predicts the mask used to upsample the predicted flow.
    ...
    def __init__(self, *, in_channels, hidden_size, multiplier=0.25):
        ...
        # 8 * 8 * 9: the predicted flow is downsampled by 8 by the feature encoder, and each
        # output pixel interpolates over its 9 neighbouring low-resolution cells.
        self.conv = nn.Conv2d(hidden_size, 8 * 8 * 9, 1, padding=0)

        # The released reference keeps this 0.25 factor to downweight the gradients of this
        # branch; it has no measurable effect on the end-point error.
...
class CorrBlock(nn.Module):
    ...
    are within a ``radius``, according to the infinity norm (see the RAFT paper section 3.2).
    """
    ...
    def __init__(self, *, num_levels: int = 4, radius: int = 4):
        ...
        # a square with sides of length 2 * radius + 1.
    ...
    def build_pyramid(self, fmap1, fmap2):
        ...
        # The feature maps are down-sampled (num_levels - 1) times by a factor of 2. The smallest
        # level needs at least 2 values per axis, otherwise grid_sample() would produce nans.
    ...
    def index_pyramid(self, centroids_coords):
        ...
        di = tensorplay.linspace(-self.radius, self.radius, neighborhood_side_len)
        dj = tensorplay.linspace(-self.radius, self.radius, neighborhood_side_len)
        delta = tensorplay.stack(list(tensorplay.meshgrid([di, dj], indexing="ij")), dim=-1).to(centroids_coords.device)
        ...
        corr_features = tensorplay.cat(indexed_pyramid, dim=-1).permute(0, 3, 1, 2).contiguous()
    ...
    def _compute_corr_volume(self, fmap1, fmap2):
        ...
        corr = tensorplay.matmul(fmap1.transpose(1, 2), fmap2)
        corr = corr.view(batch_size, h, w, 1, h, w)
        return corr / tensorplay.sqrt(tensorplay.tensor(num_channels, dtype=fmap1.dtype))
...
class RAFT(nn.Module):
    def __init__(self, *, feature_encoder, context_encoder, corr_block, update_block, mask_predictor=None):
        ...
        args:
            ...
            context_encoder (nn.Module): The context encoder. It must downsample the input by 8.
                Its input is ``image1``. Its output will be split into 2 parts:
    ...
    def forward(self, image1, image2, num_flow_updates: int = 12):
        ...
        fmaps = self.feature_encoder(tensorplay.cat([image1, image2], dim=0))
        fmap1, fmap2 = tensorplay.chunk(fmaps, 2, dim=0)
        ...
        # The context encoder output is split in 2 parts: one initializes the hidden state of the
        # recurrent units of the update block, the rest is the actual context.
        ...
        hidden_state, context = context_out.split([hidden_state_size, out_channels_context], dim=1)
        hidden_state = tensorplay.tanh(hidden_state)
        ...
        for _ in range(num_flow_updates):
            coords1 = coords1.detach()  # Gradients do not flow through the lookup branch.
    ...
    C_T_V1 = Weights(
        ...
        meta={
            **_COMMON_META,
            ...
            "_ops": 211.007,
            "_file_size": 20.129,
            "_docs": """These weights are trained on the FlyingChairs and
            FlyingThings3D datasets.""",
        },
    )
    ...
    C_T_V2 = Weights(
        ...
        meta={
            **_COMMON_META,
            ...
            "_ops": 211.007,
            "_file_size": 20.129,
            "_docs": """These weights are trained from scratch on the
            FlyingChairs and FlyingThings3D datasets.""",
        },
    )
    ...
    C_T_SKHT_V1 = Weights(
        ...
        meta={
            **_COMMON_META,
            ...
            "_file_size": 20.129,
            "_docs": """
                These weights are trained on the FlyingChairs and
                FlyingThings3D datasets and fine-tuned on Sintel. The Sintel
                fine-tuning step is a combination of the Sintel, KittiFlow,
                HD1K, and FlyingThings3D (clean pass) datasets.
            """,
        },
    )
    ...
    C_T_SKHT_V2 = Weights(
        ...
        meta={
            **_COMMON_META,
            ...
            "_file_size": 20.129,
            "_docs": """
                These weights are trained from scratch. They are pre-trained
                on the FlyingChairs and FlyingThings3D datasets and then
                fine-tuned on Sintel. The Sintel fine-tuning step is a
                combination of the Sintel, KittiFlow, HD1K, and FlyingThings3D
                (clean pass) datasets.
            """,
        },
    )
    ...
    C_T_SKHT_K_V1 = Weights(
        ...
        meta={
            **_COMMON_META,
            ...
            "_file_size": 20.129,
            "_docs": """
                These weights are pre-trained on the FlyingChairs and
                FlyingThings3D datasets, fine-tuned on Sintel, and then
                fine-tuned on the KittiFlow dataset. The Sintel fine-tuning
    ...
    C_T_SKHT_K_V2 = Weights(
        ...
        meta={
            **_COMMON_META,
            ...
            "_docs": """
                These weights are trained from scratch. They are pre-trained
                on the FlyingChairs and FlyingThings3D datasets, fine-tuned on
                Sintel, and then fine-tuned on the KittiFlow dataset. The
                Sintel fine-tuning step was described above.
            """,
        },
    )
...
def raft_large(*, weights: Optional[Raft_Large_Weights] = None, progress=True, **kwargs) -> RAFT:
    ...
    Args:
        weights(:class:`~tensorplay.vision.models.optical_flow.Raft_Large_Weights`, optional): The
            pretrained weights to use. See
            :class:`~tensorplay.vision.models.optical_flow.Raft_Large_Weights`
            below for more details, and possible values. By default, no
            pre-trained weights are used.
        progress (bool): If True, displays a progress bar of the download to stderr. Default is True.
        **kwargs: parameters passed to the ``tensorplay.vision.models.optical_flow.RAFT``
            base class. Please refer to the source code
            for more details about this class.

    .. autoclass:: tensorplay.vision.models.optical_flow.Raft_Large_Weights
        :members:
    """
...
def raft_small(*, weights: Optional[Raft_Small_Weights] = None, progress=True, **kwargs) -> RAFT:
    ...
    Args:
        weights(:class:`~tensorplay.vision.models.optical_flow.Raft_Small_Weights`, optional): The
            pretrained weights to use. See
            :class:`~tensorplay.vision.models.optical_flow.Raft_Small_Weights`
            below for more details, and possible values. By default, no
            pre-trained weights are used.
        progress (bool): If True, displays a progress bar of the download to stderr. Default is True.
        **kwargs: parameters passed to the ``tensorplay.vision.models.optical_flow.RAFT``
    ...
    .. autoclass:: tensorplay.vision.models.optical_flow.Raft_Small_Weights
        :members:
    """
