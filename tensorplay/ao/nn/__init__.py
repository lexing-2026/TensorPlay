...
tensors, ``dynamic`` hosts modules that quantize activations at runtime, and
``qat`` hosts float modules carrying simulated quantization for training.
``intrinsic`` provides fused building blocks (for example linear followed by
relu) used by the fusion workflow.
