...
def _zeropower_via_newtonschulz(
    ...
    for _ in range(ns_steps):
        ...
        gram_update = tp.addmm(
            gram_matrix, gram_matrix, gram_matrix, beta=b, alpha=c
        )
        ortho_grad = tp.addmm(ortho_grad, gram_update, ortho_grad, beta=a)
    ...
    def _init_group(
        ...
        for p in group["params"]:
            ...
            if "momentum_buffer" not in state:
                state["momentum_buffer"] = tp.zeros_like(
                    p.grad
                )
...
Muon.__doc__ = (
    ...
    + rf"""
    Args:
        params. Note that Muon is an optimizer for 2D parameters of neural network hidden layers. Other
