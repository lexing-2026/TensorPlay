...
class Bernoulli(ExponentialFamily):
    ...
    def sample(self, sample_shape=tensorplay.Size()):
        ...
        with tensorplay.no_grad():
            return tensorplay.bernoulli(self.probs.expand(shape))
    ...
    def enumerate_support(self, expand=True):
        values = tensorplay.arange(2, dtype=self._param.dtype, device=self._param.device)
    ...
    def _natural_params(self) -> tuple[Tensor]:
        return (tensorplay.logit(self.probs),)

    # pyrefly: ignore [bad-override]
    def _log_normalizer(self, x):
        return tensorplay.log1p(tensorplay.exp(x))
