...
from tensorplay.distributions import Beta, constraints
...
class LKJCholesky(Distribution):
    ...
    **References**
    ...
    def __init__(
        ...
        event_shape = tensorplay.Size((dim, dim))
        ...
        offset = tensorplay.arange(
            self.dim - 1,
            dtype=self.concentration.dtype,
            device=self.concentration.device,
        )
        offset = tensorplay.cat([offset.new_zeros((1,)), offset])
    ...
    def sample(self, sample_shape=tensorplay.Size()):
        ...
        w = tensorplay.sqrt(y) * u_hypersphere
        # Fill diagonal elements; clamp for numerical stability
        eps = tensorplay.finfo(w.dtype).tiny
        diag_elems = tensorplay.clamp(1 - tensorplay.sum(w**2, dim=-1), min=eps).sqrt()
        w += tensorplay.diag_embed(diag_elems)
        return w
    ...
    def log_prob(self, value):
        ...
        order = tensorplay.arange(2, self.dim + 1, device=self.concentration.device)
        order = 2 * (self.concentration - 1).unsqueeze(-1) + self.dim - order
        unnormalized_log_pdf = tensorplay.sum(order * diag_elems.log(), dim=-1)
        # Compute normalization constant (page 1999 of [1])
        dm1 = self.dim - 1
        alpha = self.concentration + 0.5 * dm1
        denominator = tensorplay.lgamma(alpha) * dm1
        numerator = tensorplay.mvlgamma(alpha - 0.5, dm1)
