...
from tensorplay.distributions import Categorical, constraints
from tensorplay.distributions.constraints import MixtureSameFamilyConstraint
...
class MixtureSameFamily(Distribution):
    ...
    Args:
        mixture_distribution: `tensorplay.distributions.Categorical`-like
            instance. Manages the probability of selecting components.
            The number of categories must match the rightmost batch
            dimension of the `component_distribution`. Must have either
            scalar `batch_shape` or `batch_shape` matching
            `component_distribution.batch_shape[:-1]`
        component_distribution: `tensorplay.distributions.Distribution`-like
    ...
    def __init__(
        ...
        if not isinstance(self._mixture_distribution, Categorical):
            raise ValueError(
                ...
                " instance of tensorplay.distributions.Categorical"
            )

        if not isinstance(self._component_distribution, Distribution):
            raise ValueError(
                "The Component distribution needs to be an "
                "instance of tensorplay.distributions.Distribution"
            )
    ...
    def cdf(self, x):
        ...
        return tensorplay.sum(cdf_x * mix_prob, dim=-1)
    ...
    def log_prob(self, x):
        ...
        log_mix_prob = tensorplay.log_softmax(
            self.mixture_distribution.logits, dim=-1
        )  # [B, k]
        return tensorplay.logsumexp(log_prob_x + log_mix_prob, dim=-1)  # [S, B]
    ...
    def sample(self, sample_shape=tensorplay.Size()):
        with tensorplay.no_grad():
            ...
            mix_sample_r = mix_sample.reshape(
                mix_shape + tensorplay.Size([1] * (len(es) + 1))
            )
            mix_sample_r = mix_sample_r.repeat(
                tensorplay.Size([1] * len(mix_shape)) + tensorplay.Size([1]) + es
            )

            samples = tensorplay.gather(comp_samples, gather_dim, mix_sample_r)
    ...
    def _pad_mixture_dimensions(self, x):
        ...
        x = x.reshape(
            xs[:-1]
            + tensorplay.Size(pad_ndims * [1])
            + xs[-1:]
            + tensorplay.Size(self._event_ndims * [1])
        )
        return x
