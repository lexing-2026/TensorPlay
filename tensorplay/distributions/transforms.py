...
from tensorplay.nn.functional import pad, softplus
from tensorplay.distributions._types import _Number
...
class Transform:
    ...
    :class:`tensorplay.distributions.TransformedDistribution`.
    ...
    Attributes:
        domain (:class:`~tensorplay.distributions.constraints.Constraint`):
            The constraint representing valid inputs to this transform.
        codomain (:class:`~tensorplay.distributions.constraints.Constraint`):
...
class ComposeTransform(Transform):
    ...
    def log_abs_det_jacobian(self, x, y):
        if not self.parts:
            return tensorplay.zeros_like(x)
...
class ReshapeTransform(Transform):
    ...
    elements, just as for :meth:`tensorplay.Tensor.reshape`.
    ...
    def __init__(
        self,
        in_shape: tensorplay.Size,
        out_shape: tensorplay.Size,
        cache_size: int = 0,
    ) -> None:
        self.in_shape = tensorplay.Size(in_shape)
        self.out_shape = tensorplay.Size(out_shape)
...
class PowerTransform(Transform):
    ...
    def forward_shape(self, shape):
        return tensorplay.broadcast_shapes(shape, getattr(self.exponent, "shape", ()))
...
def _clipped_sigmoid(x):
    finfo = tensorplay.finfo(x.dtype)
    return tensorplay.clamp(tensorplay.sigmoid(x), min=finfo.tiny, max=1.0 - finfo.eps)
...
class SigmoidTransform(Transform):
    ...
    def _inverse(self, y):
        finfo = tensorplay.finfo(y.dtype)
...
class AffineTransform(Transform):
    ...
    def log_abs_det_jacobian(self, x, y):
        ...
        if isinstance(scale, _Number):
            result = tensorplay.full_like(x, math.log(abs(scale)))
        else:
            result = tensorplay.abs(scale).log()
        if self.event_dim:
    ...
    def forward_shape(self, shape):
        return tensorplay.broadcast_shapes(
...
class CorrCholeskyTransform(Transform):
    ...
    def _call(self, x):
        x = tensorplay.tanh(x)
        eps = tensorplay.finfo(x.dtype).eps
        ...
        r = r + tensorplay.eye(r.shape[-1], dtype=r.dtype, device=r.device)
    ...
    def _inverse(self, y):
        ...
        y_cumsum = 1 - tensorplay.cumsum(y * y, dim=-1)
...
class StickBreakingTransform(Transform):
    ...
    def _inverse(self, y):
        ...
        sf = tensorplay.clamp(sf, min=tensorplay.finfo(y.dtype).tiny)
...
class PositiveDefiniteTransform(Transform):
    ...
    def _inverse(self, y):
        y = tensorplay.linalg.cholesky(y)
...
class CatTransform(Transform):
    ...
    in a way compatible with :func:`tensorplay.cat`.
    ...
    def _call(self, x):
        ...
        return tensorplay.cat(yslices, dim=self.dim)
    ...
    def _inverse(self, y):
        ...
        for trans, length in zip(self.transforms, self.lengths):
            ...
            start = start + length  # avoid += for jit compat
        return tensorplay.cat(xslices, dim=self.dim)
    ...
    def log_abs_det_jacobian(self, x, y):
        ...
        if dim < 0:
            return tensorplay.cat(logdetjacs, dim=dim)
        else:
...
class StackTransform(Transform):
    ...
    in a way compatible with :func:`tensorplay.stack`.
    ...
    def _call(self, x):
        ...
        for xslice, trans in zip(self._slice(x), self.transforms):
            yslices.append(trans(xslice))
        return tensorplay.stack(yslices, dim=self.dim)
    ...
    def _inverse(self, y):
        ...
        for yslice, trans in zip(self._slice(y), self.transforms):
            xslices.append(trans.inv(yslice))
        return tensorplay.stack(xslices, dim=self.dim)
    ...
    def log_abs_det_jacobian(self, x, y):
        ...
        return tensorplay.stack(logdetjacs, dim=self.dim)
