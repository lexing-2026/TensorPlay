...
class Exponential(ExponentialFamily):
    ...
    def mode(self) -> Tensor:
        return tensorplay.zeros_like(self.rate)
    ...
    def __init__(
        ...
        batch_shape = tensorplay.Size() if isinstance(rate, _Number) else self.rate.size()
    ...
    def cdf(self, value):
        if self._validate_args:
            self._validate_sample(value)
        return 1 - tensorplay.exp(-self.rate * value)

    def icdf(self, value):
        return -tensorplay.log1p(-value) / self.rate

    def entropy(self):
        return 1.0 - tensorplay.log(self.rate)
