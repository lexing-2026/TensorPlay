...
def _log_modified_bessel_fn(x, order=0):
    ...
    result = tensorplay.where(x < 3.75, small, large)
    return result


@tensorplay.graph.script_if_tracing
def _rejection_sample(loc, concentration, proposal_r, x):
    done = tensorplay.zeros(x.shape, dtype=tensorplay.bool, device=loc.device)
    # pyrefly: ignore [bad-assignment, missing-attribute]
    while not done.all():
        u = tensorplay.rand((3,) + x.shape, dtype=loc.dtype, device=loc.device)
        u1, u2, u3 = u.unbind()
        z = tensorplay.cos(math.pi * u1)
        f = (1 + proposal_r * z) / (proposal_r + z)
        c = concentration * (proposal_r - f)
        accept = ((c * (2 - c) - u2) > 0) | ((c / u2).log() + 1 - c >= 0)
        if accept.any():
            # pyrefly: ignore [no-matching-overload]
            x = tensorplay.where(accept, (u3 - 0.5).sign() * f.acos(), x)
            done = done | accept
...
class VonMises(Distribution):
    ...
    :param tensorplay.Tensor loc: an angle in radians.
    :param tensorplay.Tensor concentration: concentration parameter
    """
    ...
    def __init__(
        ...
        event_shape = tensorplay.Size()
        super().__init__(batch_shape, event_shape, validate_args)

    def log_prob(self, value):
        if self._validate_args:
            self._validate_sample(value)
        log_prob = self.concentration * tensorplay.cos(value - self.loc)
        log_prob = (
            log_prob
            - math.log(2 * math.pi)
    ...
    def _loc(self) -> Tensor:
        return self.loc.to(tensorplay.double)

    @lazy_property
    def _concentration(self) -> Tensor:
        return self.concentration.to(tensorplay.double)
    ...
    def _proposal_r(self) -> Tensor:
        ...
        return tensorplay.where(kappa < 1e-5, _proposal_r_taylor, _proposal_r)
    ...
    def sample(self, sample_shape=tensorplay.Size()):
        ...
        x = tensorplay.empty(shape, dtype=self._loc.dtype, device=self.loc.device)
