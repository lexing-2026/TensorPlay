...
from tensorplay.distributions.multivariate_normal import _batch_mahalanobis, _batch_mv
...
def _batch_capacitance_tril(W, D):
    ...
    K = tensorplay.matmul(Wt_Dinv, W).contiguous()
    K.view(-1, m * m)[:, :: m + 1] += 1  # add identity matrix to K
    return tensorplay.linalg.cholesky(K)
...
class LowRankMultivariateNormal(Distribution):
    ...
    def __init__(
        ...
        try:
            loc_, self.cov_factor, cov_diag_ = tensorplay.broadcast_tensors(
    ...
    def scale_tril(self) -> Tensor:
        ...
        K = tensorplay.matmul(Dinvsqrt_W, Dinvsqrt_W.mT).contiguous()
        K.view(-1, n * n)[:, :: n + 1] += 1  # add identity matrix to K
        scale_tril = cov_diag_sqrt_unsqueeze * tensorplay.linalg.cholesky(K)
    ...
    def covariance_matrix(self) -> Tensor:
        covariance_matrix = tensorplay.matmul(
            self._unbroadcasted_cov_factor, self._unbroadcasted_cov_factor.mT
        ) + tensorplay.diag_embed(self._unbroadcasted_cov_diag)
    ...
    def precision_matrix(self) -> Tensor:
        ...
        A = tensorplay.linalg.solve_triangular(self._capacitance_tril, Wt_Dinv, upper=False)
        precision_matrix = (
            tensorplay.diag_embed(self._unbroadcasted_cov_diag.reciprocal()) - A.mT @ A
        )
