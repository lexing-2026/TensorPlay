...
from tensorplay.distributions.transforms import AffineTransform, ExpTransform
...
class Gumbel(TransformedDistribution):
    ...
    def __init__(
        ...
        if isinstance(loc, _Number) and isinstance(scale, _Number):
            ...
            base_dist = Uniform(
                tensorplay.full_like(self.loc, finfo.tiny),
                tensorplay.full_like(self.loc, 1 - finfo.eps),
                validate_args=validate_args,
            )
        transforms = [
            ExpTransform().inv,
            AffineTransform(loc=0, scale=-tensorplay.ones_like(self.scale)),
            ExpTransform().inv,
