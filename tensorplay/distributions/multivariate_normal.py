...
from tensorplay.distributions.utils import _standard_normal, lazy_property
...
def _batch_mv(bmat, bvec):
    ...
    """
    return tensorplay.matmul(bmat, bvec.unsqueeze(-1)).squeeze(-1)
...
def _batch_mahalanobis(bL, bx):
    ...
    M_swap = (
        tensorplay.linalg.solve_triangular(flat_L, flat_x_swap, upper=False).pow(2).sum(-2)
    )  # shape = b x c
...
def _precision_to_scale_tril(P):
    ...
    Lf = tensorplay.linalg.cholesky(tensorplay.flip(P, (-2, -1)))
    L_inv = tensorplay.transpose(tensorplay.flip(Lf, (-2, -1)), -2, -1)
    Id = tensorplay.eye(P.shape[-1], dtype=P.dtype, device=P.device)
    L = tensorplay.linalg.solve_triangular(L_inv, Id, upper=False)
    return L
...
class MultivariateNormal(Distribution):
    ...
    def __init__(
        ...
        if scale_tril is not None:
            ...
            batch_shape = tensorplay.broadcast_shapes(scale_tril.shape[:-2], loc.shape[:-1])
            ...
            batch_shape = tensorplay.broadcast_shapes(
