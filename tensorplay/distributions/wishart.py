...
from tensorplay.distributions.multivariate_normal import _precision_to_scale_tril
from tensorplay.distributions.utils import _get_tracing_state, lazy_property
...
def _mvdigamma(x: Tensor, p: int) -> Tensor:
    ...
    return tensorplay.digamma(
        x.unsqueeze(-1)
        - tensorplay.arange(p, dtype=x.dtype, device=x.device).div(2).expand(x.shape + (-1,))
    ).sum(-1)


def _clamp_above_eps(x: Tensor) -> Tensor:
    # We assume positive input for this function
    return x.clamp(min=tensorplay.finfo(x.dtype).eps)
...
class Wishart(ExponentialFamily):
    ...
    Note:
        ...
        'tensorplay.distributions.LKJCholesky' is a restricted Wishart distribution.[1]
    ...
    **References**
    ...
    def __init__(
        ...
        if isinstance(df, _Number):
            batch_shape = tensorplay.Size(param.shape[:-2])
            self.df = tensorplay.tensor(df, dtype=param.dtype, device=param.device)
        else:
            batch_shape = tensorplay.broadcast_shapes(param.shape[:-2], df.shape)
        ...
        if scale_tril is not None:
            ...
            self._unbroadcasted_scale_tril = tensorplay.linalg.cholesky(covariance_matrix)
        else:  # precision_matrix is not None
            self._unbroadcasted_scale_tril = _precision_to_scale_tril(precision_matrix)

        # Chi2 distribution is needed for Bartlett decomposition sampling
        self._dist_chi2 = tensorplay.distributions.chi2.Chi2(
            df=(
                self.df.unsqueeze(-1)
                - tensorplay.arange(
                    self._event_shape[-1],
    ...
    def expand(self, batch_shape, _instance=None):
        ...
        # Chi2 distribution is needed for Bartlett decomposition sampling
        new._dist_chi2 = tensorplay.distributions.chi2.Chi2(
            df=(
                new.df.unsqueeze(-1)
                - tensorplay.arange(
                    self.event_shape[-1],
    ...
    def precision_matrix(self) -> Tensor:
        identity = tensorplay.eye(
            self._event_shape[-1],
            device=self._unbroadcasted_scale_tril.device,
            dtype=self._unbroadcasted_scale_tril.dtype,
        )
        return tensorplay.cholesky_solve(identity, self._unbroadcasted_scale_tril).expand(
            self._batch_shape + self._event_shape
        )
    ...
    def variance(self) -> Tensor:
        ...
        return self.df.view(self._batch_shape + (1, 1)) * (
            V.pow(2) + tensorplay.einsum("...i,...j->...ij", diag_V, diag_V)
        )

    def _bartlett_sampling(self, sample_shape=tensorplay.Size()):
        ...
        i, j = tensorplay.tril_indices(p, p, offset=-1)
        noise[..., i, j] = tensorplay.randn(
            tensorplay.Size(sample_shape) + self._batch_shape + (int(p * (p - 1) / 2),),
            dtype=noise.dtype,
            device=noise.device,
        )
    ...
    def rsample(
        self, sample_shape: _size = tensorplay.Size(), max_try_correction=None
    ) -> Tensor:
        r"""
        .. warning::
        ...
        sample_shape = tensorplay.Size(sample_shape)
        ...
        if _get_tracing_state():
            ...
            for _ in range(max_try_correction):
                ...
                sample = tensorplay.where(is_singular, sample_new, sample)
    ...
    def log_prob(self, value):
        ...
        return (
            ...
            * (
            ...
            - tensorplay.mvlgamma(nu / 2, p=p)
            + (nu - p - 1) / 2 * tensorplay.linalg.slogdet(value).logabsdet
            - tensorplay.cholesky_solve(value, self._unbroadcasted_scale_tril)
    ...
    def _log_normalizer(self, x, y):
        ...
        return (y + (p + 1) / 2) * (
            -tensorplay.linalg.slogdet(-2 * x).logabsdet + _log_2 * p
        ) + tensorplay.mvlgamma(y + (p + 1) / 2, p=p)
