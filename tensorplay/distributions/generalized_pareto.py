...
import tensorplay
from tensorplay import inf, nan
from tensorplay.distributions import constraints, Distribution
...
class GeneralizedPareto(Distribution):
    ...
    def rsample(self, sample_shape=tensorplay.Size()):
        shape = self._extended_shape(sample_shape)
        u = tensorplay.rand(shape, dtype=self.loc.dtype, device=self.loc.device)
        return self.icdf(u)
    ...
    def log_prob(self, value):
        ...
        z = self._z(value)
        eq_zero = tensorplay.isclose(self.concentration, tensorplay.tensor(0.0))
        safe_conc = tensorplay.where(
            eq_zero, tensorplay.ones_like(self.concentration), self.concentration
        )
        y = 1 / safe_conc + tensorplay.ones_like(z)
        where_nonzero = tensorplay.where(y == 0, y, y * tensorplay.log1p(safe_conc * z))
        log_scale = (
            math.log(self.scale) if isinstance(self.scale, Real) else self.scale.log()
        )
        return -log_scale - tensorplay.where(eq_zero, z, where_nonzero)
    ...
    def log_survival_function(self, value):
        ...
        where_nonzero = -tensorplay.log1p(safe_conc * z) / safe_conc
        return tensorplay.where(eq_zero, -z, where_nonzero)

    def log_cdf(self, value):
        return tensorplay.log1p(-tensorplay.exp(self.log_survival_function(value)))

    def cdf(self, value):
        return tensorplay.exp(self.log_cdf(value))

    def icdf(self, value):
        loc = self.loc
        scale = self.scale
        concentration = self.concentration
        eq_zero = tensorplay.isclose(concentration, tensorplay.zeros_like(concentration))
        safe_conc = tensorplay.where(eq_zero, tensorplay.ones_like(concentration), concentration)
        logu = tensorplay.log1p(-value)
        where_nonzero = loc + scale / safe_conc * tensorplay.expm1(-safe_conc * logu)
        where_zero = loc - scale * logu
        return tensorplay.where(eq_zero, where_zero, where_nonzero)
    ...
    def mean(self):
        ...
        safe_conc = tensorplay.where(valid, concentration, 0.5)
        result = self.loc + self.scale / (1 - safe_conc)
        return tensorplay.where(valid, result, nan)

    @property
    def variance(self):
        concentration = self.concentration
        valid = concentration < 0.5
        safe_conc = tensorplay.where(valid, concentration, 0.25)
        result = self.scale**2 / ((1 - safe_conc) ** 2 * (1 - 2 * safe_conc))
        return tensorplay.where(valid, result, nan)

    def entropy(self):
        ans = tensorplay.log(self.scale) + self.concentration + 1
        return tensorplay.broadcast_to(ans, self._batch_shape)
