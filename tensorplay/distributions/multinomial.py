...
from tensorplay.distributions.binomial import Binomial
...
class Multinomial(Distribution):
    ...
    def sample(self, sample_shape=tensorplay.Size()):
        ...
        samples = self._categorical.sample(
            tensorplay.Size((self.total_count,)) + sample_shape
        )
        ...
        counts.scatter_add_(-1, samples, tensorplay.ones_like(samples))
        return counts.type_as(self.probs)

    def entropy(self):
        n = tensorplay.tensor(self.total_count)

        cat_entropy = self._categorical.entropy()
        term1 = n * cat_entropy - tensorplay.lgamma(n + 1)

        support = self._binomial.enumerate_support(expand=False)[1:]
        binomial_probs = tensorplay.exp(self._binomial.log_prob(support))
        weights = tensorplay.lgamma(support + 1)
    ...
    def log_prob(self, value):
        ...
        logits = logits.clone(memory_format=tensorplay.contiguous_format)
        log_factorial_n = tensorplay.lgamma(value.sum(-1) + 1)
        log_factorial_xs = tensorplay.lgamma(value + 1).sum(-1)
