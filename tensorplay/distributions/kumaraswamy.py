...
from tensorplay.distributions.uniform import Uniform
from tensorplay.distributions.utils import broadcast_all, euler_constant
...
def _moments(a, b, n):
    """
    Computes nth moment of Kumaraswamy using tensorplay.lgamma
    """
    arg1 = 1 + n / a
    log_value = tensorplay.lgamma(arg1) + tensorplay.lgamma(b) - tensorplay.lgamma(arg1 + b)
    return b * tensorplay.exp(log_value)
...
class Kumaraswamy(TransformedDistribution):
    ...
    def __init__(
        ...
        base_dist = Uniform(
            tensorplay.full_like(self.concentration0, 0),
            tensorplay.full_like(self.concentration0, 1),
    ...
    def variance(self) -> Tensor:
        return _moments(self.concentration1, self.concentration0, 2) - tensorplay.pow(
            self.mean, 2
        )
    ...
    def entropy(self):
        ...
        H0 = tensorplay.digamma(self.concentration0 + 1) + euler_constant
        return (
            t0
            + t1 * H0
            - tensorplay.log(self.concentration1)
            - tensorplay.log(self.concentration0)
        )
