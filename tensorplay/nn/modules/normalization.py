...
import tensorplay
from tensorplay import Size, Tensor
...
class LocalResponseNorm(Module):
    ...
    Examples::
        ...
        >>> signal_2d = tensorplay.randn(32, 5, 24, 24)
        >>> signal_4d = tensorplay.randn(16, 5, 7, 7, 7, 7)
...
class LayerNorm(Module):
    ...
    `tensorplay.var(input, unbiased=False)`.
    ...
    Args:
        normalized_shape (int or list or tensorplay.Size): input shape from an expected input
            of size
    ...
    Examples::
        ...
        >>> embedding = tensorplay.randn(batch, sentence_length, embedding_dim)
        ...
        >>> # Image Example
        >>> N, C, H, W = 20, 5, 10, 10
        >>> input = tensorplay.randn(N, C, H, W)
    ...
    def __init__(
        ...
        if self.elementwise_affine:
            self.weight = Parameter(
                tensorplay.empty(self.normalized_shape, **factory_kwargs)
            )
            if bias:
                self.bias = Parameter(
                    tensorplay.empty(self.normalized_shape, **factory_kwargs)
...
class GroupNorm(Module):
    ...
    Examples::
        ...
        >>> input = tensorplay.randn(20, 6, 10, 10)
    ...
    def __init__(
        ...
        self.affine = affine
        if self.affine:
            self.weight = Parameter(tensorplay.empty(num_channels, **factory_kwargs))
            self.bias = Parameter(tensorplay.empty(num_channels, **factory_kwargs))
        else:
            self.register_parameter("weight", None)
            self.register_parameter("bias", None)
...
class RMSNorm(Module):
    ...
    Args:
        normalized_shape (int or list or tensorplay.Size): input shape from an expected input
            ...
            If a single integer is used, it is treated as a singleton list, and this module will
            normalize over the last dimension which is expected to be of that specific size.
        eps: a value added to the denominator for numerical stability. Default: ``tensorplay.finfo(x.dtype).eps``
        elementwise_affine: a boolean value that when set to ``True``, this module
