...
from tensorplay.nn.attention import _raise_kernel_warnings
from tensorplay.nn.attention._utils import (
    _calculate_scale,
...
class CausalVariant(IntEnum):
    ...
    .. code-block:: python
        ...
        tensorplay.tril(tensorplay.ones(size, dtype=tensorplay.bool))
    ...
    .. code-block:: python
        ...
        tensorplay.tril(
            tensorplay.ones(size, dtype=tensorplay.bool),
...
class CausalBias(tensorplay.Tensor):
    """
    ...
    def _upper_left(self, device: tensorplay.device) -> tensorplay.Tensor:
        """Upper left causal bias"""
        return tensorplay.tril(
            tensorplay.ones(self.seq_len_q, self.seq_len_kv, device=device, dtype=tensorplay.bool)
        )

    def _lower_right(self, device: tensorplay.device) -> tensorplay.Tensor:
        """Lower right causal bias"""
        diagonal_offset = self.seq_len_kv - self.seq_len_q
        return tensorplay.tril(
            tensorplay.ones(
                self.seq_len_q, self.seq_len_kv, device=device, dtype=tensorplay.bool
            ),
            diagonal=diagonal_offset,
        )

    # pyrefly: ignore [bad-return]
    def _materialize(self, device: tensorplay.device | None = None) -> tensorplay.Tensor:
        """
        ...
        Args:
            device (Optional[tensorplay.device]): The device on which to create the tensor. Defaults to CPU.

        Returns:
            tensorplay.Tensor: The materialized bias tensor.
        """
        if device is None:
    ...
    @classmethod
    def __tensorplay_function__(cls, func, types, args=(), kwargs=None):
        """Defines the behavior of tensorplay.nn.functional.scaled_dot_product_attention when the attn_bias is an AttnBias"""
        if kwargs is None:
            kwargs = {}
        if func is tensorplay.nn.functional.scaled_dot_product_attention:
