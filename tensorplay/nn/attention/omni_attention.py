...
"""User-facing API for omni attention: block-sparse mask metadata and
attention with an arbitrary attention-score modification function."""
...
from tensorplay._higher_order_ops.utils import setup_compilation_env
from tensorplay.nn.attention._utils import _validate_sdpa_input
...
if typing.TYPE_CHECKING:
    from tensorplay.graph.node import BaseArgumentTypes

    DeviceLikeType = str | tensorplay.device
...
#
# This flag only affects the internal compilation when omni_attention is called directly.
# If you have already wrapped omni_attention in compile(), this flag has no effect
# and the user's compilation will still occur.
#
# Usage:
#   import tensorplay.nn.attention.omni_attention as oa
#   oa._OMNI_ATTENTION_DISABLE_COMPILE_DEBUG = True
#   # Now you can set breakpoints in your score_mod/mask_mod
#   output = oa.omni_attention(q, k, v, score_mod=my_score_mod)
#
_OMNI_ATTENTION_DISABLE_COMPILE_DEBUG = False
...
def _validate_block_mask_shape(
    query: Tensor,
    key: Tensor,
    q_len: int | tensorplay.SymInt,
    kv_len: int | tensorplay.SymInt,
    block_mask_q_len: int | tensorplay.SymInt,
    block_mask_kv_len: int | tensorplay.SymInt,
) -> None:
    """Preserve unbacked checks without specializing regular dynamic lengths."""
    # Imported lazily: the symbolic-shapes package pulls in the graph passes,
    # which import back into this package.
    from tensorplay.graph.experimental.symbolic_shapes import has_free_unbacked_symbols
    ...
    if has_unbacked_input_lengths:
        tensorplay._check(q_len <= block_mask_q_len, _BLOCK_MASK_TOO_SMALL_ERROR)
        tensorplay._check(kv_len <= block_mask_kv_len, _BLOCK_MASK_TOO_SMALL_ERROR)
        tensorplay._check(q_len >= block_mask_q_len, _BLOCK_MASK_TOO_LARGE_ERROR)
        tensorplay._check(kv_len >= block_mask_kv_len, _BLOCK_MASK_TOO_LARGE_ERROR)
        return

    # Case 2: backed lengths use regular validation so the tracer can generalize equality.
...
class OmniKernelOptions(TypedDict, total=False):
    """Options for controlling the behavior of omni attention kernels.

    These options are passed to the underlying kernels to control performance
    ...
    Example Usage:
        .. code-block:: python
            ...
            output = omni_attention(q, k, v, kernel_options=kernel_opts)

            # Using TypedDict (recommended for type safety)
            from tensorplay.nn.attention.omni_attention import OmniKernelOptions

            kernel_opts: OmniKernelOptions = {
                "BLOCK_M": 64,
                "BLOCK_N": 64,
                "PRESCALE_QK": True,
            }
    ...
    """Number of warps to use in the kernel. Higher values may improve performance
    but increase register pressure. Default is determined by autotuning."""

    num_stages: NotRequired[int]
    """Number of pipeline stages in the kernel. Higher values may improve performance
    ...
    FORCE_USE_OMNI_ATTENTION: NotRequired[bool]
    """If True, forces the use of the omni attention kernel instead of potentially using
    the more optimized decoding kernel for short sequences. This can be a helpful
    ...
    Options:
        - "AUTO": Use current heuristics (typically tiling-based kernels with
          automatic selection between omni_attention and decoding)
        - "TRITON": Standard omni_attention kernel
        - "TRITON_DECODE": Decoding kernel, only available for short sequence lengths with specific configurations
        - "FLASH": Experimental: Flash Attention kernel (cute-dsl), user needs to have flash installed

    This option cannot be combined with legacy knobs such as ``FORCE_USE_OMNI_ATTENTION``.
    Raises an error if the requested backend cannot be used. Default: "AUTO"
    """


class _KernelOptionsWithInternals(OmniKernelOptions, total=False):
    OUTPUT_LOGSUMEXP: bool
    OUTPUT_MAX: bool


class AuxRequest(NamedTuple):
    """Request which auxiliary outputs to compute from omni_attention.
...
class AuxOutput(NamedTuple):
    """Auxiliary outputs from omni_attention operation.
...
# Need to define it here so that the tracer doesn't skip it
def _vmap_for_bhqkv(
    fn: Callable[..., _R],
    ...
    """
    # We vmap a function 4 times, broadcasting the [b, h, q_idx, kv_idx] dimensions
    ...
    for dims in dimensions:
        fn = tensorplay.func.vmap(fn, in_dims=prefix + dims + suffix, out_dims=out_dims)  # type: ignore[arg-type]
    return fn
...
def noop_mask(
    ...
    return batch.new_ones(size=(), dtype=tensorplay.bool, device=batch.device)
...
def _ordered_to_dense(
    ...
    def create_dense_one(kv_num_blocks, kv_indices):
        dense_mask = kv_indices.new_zeros(num_rows, num_cols + 1, dtype=tensorplay.int32)

        row_indices = tensorplay.arange(num_rows, dtype=tensorplay.int64, device=device).unsqueeze(
            -1
        )
        col_range = tensorplay.arange(max_entries, dtype=tensorplay.int64, device=device)
        index_mask = col_range < kv_num_blocks.unsqueeze(-1)

        # We write to one spot "out of bounds"
        valid_indices = tensorplay.where(index_mask, kv_indices, num_cols)
    ...
    for _ in range(len(batch_dims)):
        create_dense_batched = tensorplay.func.vmap(create_dense_batched, in_dims=(0, 0))
...
def _dense_to_ordered(dense_mask: Tensor) -> tuple[Tensor, Tensor]:
    ...
    with tensorplay.graph.traceback.annotate({"fallback_to_eager": True}):
        col_indices = tensorplay.argsort(dense_mask, dim=-1, descending=True, stable=True)
    return (
        num_blocks_in_row.to(tensorplay.int32, memory_format=tensorplay.contiguous_format),
        col_indices.to(tensorplay.int32, memory_format=tensorplay.contiguous_format),
    )
...
def _adjust_num_blocks_and_indices(
    ...
    valid_entries = (
        tensorplay.arange(indices.shape[-1], dtype=num_blocks.dtype, device=indices.device)
        < num_blocks[..., None]
    )
    num_blocks = tensorplay.sum(valid_entries & (indices < new_num_cols), dim=-1).to(
        tensorplay.int32
    )
...
def _extract_callable_pytree(
    ...
    Skipped under graph tracing (tensorplay.compiler.is_compiling) because the
    tracer can't trace through closure cell introspection and handles freevars
    via its own lifting mechanism.
    """
    if tensorplay.compiler.is_compiling() or not (
...
class _MaskModWrapper:
    ...
    When extraction is skipped (e.g., under tracing), fn is the original
...
class BlockMask:
    ...
    **Details**
    ...
    def __getitem__(
        ...
        Example:
            .. code-block:: python
                ...
                q = tensorplay.randn(4, 2, 512, 64, device="cuda")
                q_chunk = q[:, :, :128, :]
    ...
    def __repr__(self) -> str:
        def shape_or_none(x: Tensor | None):
    ...
    def to(self, device: tensorplay.device | str) -> BlockMask:
        """Moves the BlockMask to the specified device.

        Args:
            device (tensorplay.device or str): The target device to move the BlockMask to.
                Can be a device object or a string (e.g., 'cpu', 'cuda:0').
...
def _convert_mask_to_block_mask(
    ...
    mask = tensorplay.nn.functional.pad(
        mask,
        (
            0,
    ...
    if separate_full_blocks:
        ...
        partial_blocks = partial_blocks.to(dtype=tensorplay.int8)
        full_blocks = full_blocks.to(dtype=tensorplay.int8)
...
def or_masks(*mask_mods: _mask_mod_signature) -> _mask_mod_signature:
    ...
    def or_mask(b: Tensor, h: Tensor, q_idx: Tensor, kv_idx: Tensor) -> Tensor:
        result = b.new_zeros((), dtype=tensorplay.bool)
...
def and_masks(*mask_mods: _mask_mod_signature) -> _mask_mod_signature:
    ...
    def and_mask(b: Tensor, h: Tensor, q_idx: Tensor, kv_idx: Tensor) -> Tensor:
        result = b.new_ones((), dtype=tensorplay.bool)
        for mask in mask_mods:
            result = result & mask(b, h, q_idx, kv_idx)
        return result
...
def create_mask(
    ...
    """
    if device is None:
        device = tensorplay.accelerator.current_accelerator() or "cpu"
    if B is None:
        B = 1
    if H is None:
        H = 1
    b = tensorplay.arange(0, B, device=device)
    h = tensorplay.arange(0, H, device=device)
    m = tensorplay.arange(0, Q_LEN, device=device)
    ...
    with TransformGetItemToIndex():
        if mod_type == _ModificationType.SCORE_MOD:
            ...
            out = score_mod(tensorplay.zeros(B, H, Q_LEN, KV_LEN, device=device), b, h, m, n)
            mask = tensorplay.where(tensorplay.isneginf(out), False, True)
            return mask
...
def _compute_dq_write_order_from_block_mask(
    ...
    B, H = tensorplay.broadcast_shapes(*broadcast_shapes)
    ...
    rank_table = (cumsum - dense).to(tensorplay.int32)

    if dq_kv_order:
        total_per_m = cumsum[:, :, :, -1:]
        rank_table = (total_per_m - 1 - rank_table).to(tensorplay.int32)

    def _gather_write_order(bwd_idx: Tensor) -> Tensor:
        b_i = tensorplay.arange(B, device=device)[:, None, None, None]
        h_i = tensorplay.arange(H, device=device)[None, :, None, None]
        n_i = tensorplay.arange(bwd_idx.shape[2], device=device)[None, None, :, None]
        m_vals = bwd_idx.long().clamp(0, num_m - 1)
        return rank_table[b_i, h_i, m_vals, n_i].to(tensorplay.int32)
...
def create_block_mask(
    ...
    Args:
        ...
        compute_dq_write_order (bool): If True, precompute dQ write-order
            metadata needed by deterministic block-sparse backward.
    ...
    if _compile:
        warnings.warn(
            "_compile flag on create_block_mask was originally added to work around a compile limitation. That limitation has since been addressed. So, to compile create_block_mask, we suggest doing compile(create_block_mask). This still works for now, but will be removed in the future.",
            DeprecationWarning,
            stacklevel=2,
        )
        return tensorplay.compile(create_block_mask)(
            mask_mod,
            B,
            H,
            Q_LEN,
...
def _create_empty_block_mask(query: Tensor, key: Tensor) -> BlockMask:
    r"""Default block mask for omni attention.
    ...
    device = query.device
    return BlockMask.from_kv_blocks(
        kv_num_blocks=tensorplay.ones(1, 1, 1, dtype=tensorplay.int32, device=device),
        kv_indices=tensorplay.zeros(1, 1, 1, 1, dtype=tensorplay.int32, device=device),
...
def _apply_kernel_options(
    ...
    value: Tensor,
    return_lse: bool,
    kernel_options: OmniKernelOptions | None,
    ...
    if "BACKEND" in kernel_options and kernel_options.get(
        "FORCE_USE_OMNI_ATTENTION", False
    ):
        # TODO: remove FORCE_USE_OMNI_ATTENTION once BACKEND is fully adopted.
        raise RuntimeError(
            "BACKEND cannot be combined with legacy FORCE_USE_OMNI_ATTENTION. "
            "BACKEND supersedes the legacy knob; please drop FORCE_USE_OMNI_ATTENTION "
    ...
    if not output_lse:
        ...
        kernel_options["OUTPUT_LOGSUMEXP"] = tensorplay.is_grad_enabled()
...
def _validate_device(query: Tensor, key: Tensor, value: Tensor) -> None:
    ...
    if query.device.type == "cpu" and (
        ...
        raise NotImplementedError(
            "OmniAttention does not support backward on CPU. Please set the input requires_grad to False or use another device."
        )
    if query.device.type == "mps" and (
        query.requires_grad or key.requires_grad or value.requires_grad
    ):
        raise NotImplementedError(
            "OmniAttention does not support backward on MPS. Please set the input requires_grad to False or use another device."
        )
    supported_devices = {"cuda", "cpu", "xpu", "hpu", "mps"}
    if query.device.type not in supported_devices:
        raise ValueError(
            "OmniAttention is only supported on CUDA, CPU, HPU, or MPS devices. "
...
def _validate_no_nested_tensors(query: Tensor, key: Tensor, value: Tensor) -> None:
    if query.is_nested() or key.is_nested() or value.is_nested():
        raise NotImplementedError(
            ...
            "Convert inputs to dense tensors before calling omni_attention."
        )
...
def _enforce_mem_layouts(
    ...
    fp8_dtypes = (
        tensorplay.float8_e4m3fn,
        tensorplay.float8_e5m2,
    )
    gemm_precision = query.dtype

    should_enforce_mem_layout = (
        gemm_precision in fp8_dtypes
        and tensorplay.version.cuda is not None
        and tensorplay.cuda.get_device_capability("cuda") >= (8, 9)
        and tensorplay.cuda.get_device_capability("cuda") < (10, 0)
    )
...
@overload
@deprecated(
    "return_lse is deprecated and will be removed in a future release. "
...
def omni_attention(
    ...
    kernel_options: OmniKernelOptions | None = None,
    *,
    return_aux: AuxRequest | None = None,
    ...
    Where:
        ...
        - ``batch``, ``head``, ``q_idx``, ``k_idx``: Scalar tensors indicating
          ...
          These should have the ``tensorplay.int64`` data type and be located on the same device as the score tensor.
    ...
    Args:
        ...
        kernel_options (Optional[OmniKernelOptions]):
            Options to control the behavior of the underlying kernels.
            See :class:`OmniKernelOptions` for available options and usage examples.
    ...
    .. warning::
        `tensorplay.nn.attention.omni_attention` is a prototype feature.
        Please look forward to a more stable implementation in a future version.
    """
    ...
    if not _OMNI_ATTENTION_DISABLE_COMPILE_DEBUG:
        _warn_once(
            warning_id="omni_attention_performance",
            message=(
                "omni_attention called without compile() - this will use an unfused implementation that materializes the full scores matrix instead of generating a fused kernel.\n\n"
                "SOLUTION: Use compile(omni_attention)(...)\n\n"
    ...
    with setup_compilation_env() as backend:
        if _OMNI_ATTENTION_DISABLE_COMPILE_DEBUG:
            omni_fn = _omni_attention_hop_wrapper
        else:
            omni_fn = tensorplay.compile(
    ...
    return _finalize_outputs(
        ...
        return_aux=return_aux,
        return_lse=return_lse,
        stats_are_log2=_OMNI_ATTENTION_DISABLE_COMPILE_DEBUG
