"""Parametrization of tensors living on a module.

A parametrization is a map ``f`` applied on the fly to a parameter or
buffer stored on a module, so that ``module.weight`` returns
``f(module.parametrizations.weight.original)`` while the optimizer keeps
seeing the unparametrized ``original``.  Gradients flow through ``f``
during the backward pass, so the constraint it encodes is maintained at
every optimization step.

The mechanism is built out of three pieces:

1. The original tensor is moved into a :class:`ParametrizationList`, a
   sequential container registered under ``module.parametrizations``.
2. A property named after the tensor is injected on a dynamically
   created subclass of the module, forwarding attribute access to the
   parametrization list.  Assignment goes through ``right_inverse``.
3. A global cache, active within the :func:`cached` context manager,
   computes every parametrized tensor once per forward pass.
"""
...
from tensorplay.nn.modules.container import Module, ModuleDict, ModuleList
from tensorplay.nn.parameter import Parameter
from tensorplay.utils._dispatch import is_traceable_wrapper_subclass
...
def _maybe_set(dest: Tensor, src: Tensor) -> None:
    should_swap = (
        tensorplay.__future__.get_swap_module_params_on_conversion()
        or is_traceable_wrapper_subclass(dest)
    )
    if should_swap:
        if isinstance(dest, Parameter) and not isinstance(src, Parameter):
            src = Parameter(src, requires_grad=dest.requires_grad)
        tensorplay.utils.swap_tensors(dest, src)
    else:
        dest.set_(src)


class ParametrizationList(ModuleList):
    r"""A sequential container that holds and manages the original parameters or buffers of a parametrized :class:`tensorplay.nn.Module`.
    ...
    def __init__(
        ...
        #           (isinstance(Y, Sequence) and all(isinstance(t, Tensor) for t in Y))
            ...
            for i, originali in enumerate(new):
                if not isinstance(originali, Tensor):
                    raise ValueError(
                        ...
                        f"(list, tuple...). Got element {i} of the sequence with type "
...
def register_parametrization(
    ...
    *,
    ...
    Examples:
        ...
        >>> print(tp.allclose(m.weight, m.weight.T))  # m.weight is now symmetric
        True
        >>> A = tp.rand(5, 5)
        >>> A = A + A.T  # A is now symmetric
        >>> m.weight = A  # Initialize the weight to be the symmetric matrix A
        >>> print(tp.allclose(m.weight, A))
        True
        ...
        ...         U, S, Vh = tp.linalg.svd(Z, full_matrices=False)
        ...
        ... )
        >>> print(tp.linalg.matrix_rank(linear_rank_one.weight).item())
        1
    """
...
def remove_parametrizations(
    ...
    if parametrizations.is_tensor:
        ...
        if leave_parametrized:
            ...
            with tensorplay.no_grad():
                if type(original) is Tensor:
                    _maybe_set(original, t)
                else:
                    try:
                        _maybe_set(original, t)
                    except RuntimeError as e:
                        # A parameter coming from a tensor subclass needs a working
                        # set_() implementation to be moved into in-place.
                        raise RuntimeError(
                            ...
                            "tensorplay.__future__.set_swap_module_params_on_conversion(True)."
                        ) from e
    else:
        if leave_parametrized:
            ...
            # Wrapping in a Parameter detaches, since a registered parameter
            # must be a leaf.
