...
class GlobGroup:
    ...
    A candidate is composed of a list of segments separated by ``separator``,
    e.g. ``"foo.bar.baz"``.

    A pattern contains one or more segments. Segments can be:
        - A literal string (e.g. ``"foo"``), which matches exactly.
        - A string containing a wildcard (e.g. ``"tensorplay*"``, or
          ``"foo*baz*"``). The wildcard matches any string, including the
          empty string.
        - A double wildcard (``"**"``). This matches against zero or more
          complete segments.

    Examples:
        ``tensorplay.**``: matches ``tensorplay`` and all its submodules, e.g.
        ``tensorplay.nn`` and ``tensorplay.nn.functional``.
        ``tensorplay.*``: matches ``tensorplay.nn`` or ``tensorplay.utils``,
        but not ``tensorplay.nn.functional``.
        ``tensorplay*.**``: matches ``tensorplay``, ``tensorplay_vision``, and
        all their submodules.

    A candidate will match the ``GlobGroup`` if it matches any of the
    ...
    Args:
        ...
        separator (str): A string that delimits segments in candidates and
            patterns. By default this is "." which corresponds to how modules
            are named in Python. Another common value for this is "/", which
            is the Unix path separator.
    """
    ...
    def _glob_to_re(pattern: str, separator: str = "."):
        # To avoid corner cases for the first component, the candidate string
        # is prefixed with the separator so that e.g. `import tensorplay`
        # matches against `.tensorplay` (assuming '.' is the separator).
