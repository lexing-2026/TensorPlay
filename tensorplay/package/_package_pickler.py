...
class _PassthroughPickler(_Pickler):
    """A plain pickler whose ``persistent_id`` can be replaced at runtime.

    The stock pickler resolves ``persistent_id`` on every ``save`` call, so
    instances of this class can have the attribute swapped for a callable
    provided by, e.g., the package exporter.
    """
...
class PackagePickler(_PassthroughPickler):
    ...
    def __init__(self, importer: Importer, *args, **kwargs):
        ...
        # Make sure the dispatch table taken from _Pickler is up-to-date.
    ...
    def save_global(self, obj, name=None):
        # The stock pickler factors this code in a way that requires
        # re-implementing it here. The only behavioral change is marked
        # with CHANGED below.
...
def create_pickler(data_buf, importer, protocol=4):
    if importer is sys_importer:
        ...
        # we can use the plain pickler implementation, which
        # resolves globals through the standard import system
        return _PassthroughPickler(data_buf, protocol=protocol)
    else:
