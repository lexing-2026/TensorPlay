...
from tensorplay.types import FileLike

from ._archive import PackageFileReader
...
# been marked as extern. This is to work around the fact that many numeric
# libraries implicitly depend on numpy and dependency scanning can't track it.
IMPLICIT_IMPORT_ALLOWLIST: Iterable[str] = [
    "numpy",
    "numpy.core",
    "numpy.core._multiarray_umath",
    # Some code generation tools emit references to the builtins module and
    # users usually don't extern builtins. Here we import it here by default.
    "builtins",
]
...
def _maybe_decode_ascii(data: Any) -> Any:
    """Decode bytes payloads carried inside persistent-load ids when needed."""
...
class PackageImporter(Importer):
    ...
    for the packaging of model code and data so that it can be run
    ...
    def __init__(
        self,
        file_or_buffer: FileLike | PackageFileReader,
        ...
        """
        self.zip_reader: Any
        if isinstance(file_or_buffer, PackageFileReader):
            ...
            if not os.path.isdir(self.filename):
                self.zip_reader = PackageFileReader(self.filename)
            else:
                self.zip_reader = DirectoryReader(self.filename)
        else:
            self.filename = "<binary>"
            self.zip_reader = PackageFileReader(file_or_buffer)
        ...
        self.modules["tensorplay_package_importer"] = self  # type: ignore[assignment]
        ...
        # used by callers that want a pickle factory bound to this importer
    ...
    def load_pickle(self, package: str, resource: str, map_location=None) -> Any:
        ...
        Args:
            package (str): The name of module package (e.g. ``"my_package.my_subpackage"``).
            resource (str): The unique name for the resource.
            map_location: Retained for interface compatibility; tensorplay embeds
                tensor payloads directly in the pickle stream, so no remapping of
                storage records takes place. Defaults to ``None``.
        ...
        def persistent_load(saved_id):
            ...
            if typename == "reduce_package":
                ...
                raise NotImplementedError(
                    "This package archive contains raw storage records, which "
                    "tensorplay.package does not produce or load. Tensors saved "
                    "with save_pickle embed their payload in the pickle stream."
                )
            else:
                raise RuntimeError(
                    f"Unknown typename for persistent_load, expected 'storage' or "
                    f"'reduce_package' but got '{typename}'"
                )

        # Load the data (which may in turn use `persistent_load`)
    ...
    def id(self):
        """
        Returns internal identifier that tensorplay.package uses to distinguish :class:`PackageImporter` instances.
        Looks like::
    ...
    def _make_module(
        ...
        ns["__tensorplay_package__"] = True
    ...
    # note: behavior follows the standard import machinery's find-and-load
    # sequence, with module creation routed through _make_module
    ...
    def _find_and_load(self, name):
        ...
        # To handle libraries that create fake submodules by inserting them
        # directly into a modules registry, where those submodules are not
        # import-friendly through the normal lookup path.
        if name == "os":
