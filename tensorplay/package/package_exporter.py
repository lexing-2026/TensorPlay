...
from ._archive import PackageFileWriter
...
class _ModuleProviderAction(Enum):
    ...
    # (`tensorplay_package_importer`) that allows packaged code to access it.
    # Don't re-export this.
    SKIP = 6
...
class PackagingErrorReason(Enum):
    ...
    IS_EXTENSION_MODULE = (
        "Module is a C extension module. tensorplay.package supports Python modules only."
    )
...
class PackageExporter:
    ...
    Only plain Python modules are supported: C extension modules and other
    compiled or interpreter-specific artifacts cannot be interned. Objects
    embedded with :meth:`save_pickle` serialize tensors through tensorplay's
    own pickling support.
    """
    ...
    def __init__(
        ...
        self.zip_file = PackageFileWriter(f)
    ...
    def add_dependency(self, module_name: str, dependencies=True):
        ...
        # `tensorplay_package_importer` that allows packaged modules to reference
        # their PackageImporter. We don't want to re-export this.
        if module_name == "tensorplay_package_importer":
            self.dependency_graph.add_node(
                module_name,
    ...
    def save_pickle(
        ...
        """Save a python object to the archive using pickle. Equivalent to :func:`tensorplay.save` but saving into
        ...
        if pickle_protocol not in (3, 4):
            raise AssertionError(
                f"tensorplay.package only supports pickle protocols 3 and 4, got {pickle_protocol}"
            )
        ...
        *,
        ...
        Args:
            include (list[str] | str): A string e.g. ``"my_package.my_subpackage"``, or list of strings
                ...
                Examples :
                    ``'tensorplay.**'`` -- matches ``tensorplay`` and all submodules of tensorplay, e.g. ``'tensorplay.nn'``
                    and ``'tensorplay.nn.functional'``

                    ``'tensorplay.*'`` -- matches ``'tensorplay.nn'`` or ``'tensorplay.functional'``, but not
                    ``'tensorplay.nn.functional'``

            exclude (list[str] | str): An optional pattern that excludes some patterns that match the include string.
                e.g. ``include='tensorplay.**', exclude='tensorplay.foo'`` will mock all tensorplay packages except ``'tensorplay.foo'``,
                Default: is ``[]``.
    ...
    def _persistent_id(self, obj):
        if tensorplay.is_storage(obj):
            # tensorplay tensors embedded in pickled objects are serialized by
            # tensorplay's own pickling support. Writing raw storage records
            # into the package archive is not supported.
            raise NotImplementedError(
                "Serializing a bare storage object into a package is not supported. "
                "Package tensors (or containers holding them) instead; their data "
                "is embedded in the pickle stream by tensorplay's serialization."
            )
    ...
    def _write(self, filename, str_or_bytes):
        ...
        if is_mangled(filename):
            raise AssertionError(
                f"Tried to save a tensorplay.package'd module as '{filename}'. "
                "Directly saving tensorplay.package'd modules is not allowed."
            )
    ...
    def _execute_dependency_graph(self):
        ...
        for module_name, attrs in self.dependency_graph.nodes.items():
            ...
            if action == _ModuleProviderAction.EXTERN:
                ...
                raise AssertionError(
                    f"Invalid action: {module_name}, {action}. Please report a bug to the tensorplay maintainers."
                )
    ...
    def _can_implicitly_extern(self, module_name: str):
        ...
        return top_level_package_name == "tensorplay" or (
