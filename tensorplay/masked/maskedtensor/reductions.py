...
def _masked_all_dim(data, dim, keepdim=False, mask=None):
    if mask is None:
        return tp.all(data, dim=dim, keepdim=keepdim)
    return tp.all(data.masked_fill(~mask, True), dim=dim, keepdim=keepdim)
...
def _multidim_any(mask, dim, keepdim):
    ...
    for d in sorted(dim, reverse=True):
        mask = tp.any(mask, dim=d, keepdim=keepdim)
    return mask
...
def _get_masked_fn(fn):
    if fn == "all":
        return _masked_all
    from tensorplay import masked

    return getattr(masked, fn)
...
def _torch_reduce_all(fn):
    def reduce_all(self):
        ...
        # When reduction is "all", then tp.argmin/tp.argmax needs to return the index of the
        ...
        if fn == "all":
            ...
            sparse_idx = masked_fn(data.values(), mask=mask).to(dtype=tp.int64)
            indices = (
                data.to_sparse().indices()
                if not self.is_sparse_coo()
                else data.indices()
            )
            idx = indices.unbind(1)[sparse_idx]
            stride = data.size().numel() / tp.tensor(
                data.size(), device=data.device
            ).cumprod(0)
            result_data = tp.sum(idx * stride)
        ...
        return as_masked_tensor(result_data, tp.any(mask))
    ...
    def reduce_dim(self, dim, keepdim=False, dtype=None):
        if self.is_sparse:
            msg = (
                ...
                "If you would like this operator to be supported, please file an issue for a feature request "
                "with a minimal reproducible code snippet.\n"
...
def _torch_grad_reduce(fn):
    def grad_reduce(*args, **kwargs):
        if len(args) == 1 and len(kwargs) == 0:
            return _torch_reduce_all(fn)(args[0])
        # The custom autograd function of this package does not accept keyword
        # arguments, so positional application is used here.
...
def _build_reduce_maps():
    """Build the reduction lookup tables.

    Reductions are reachable both as module functions and as tensor methods;
    the module function is the stable, hashable identity used as the table
    key, and the explicit methods on MaskedTensor route through it. The
    native-operator table and the function-keyed table therefore share the
    same keys, with the native table taking precedence on lookup.
    """
    native_map = {}
    function_map = {}
    for name in REDUCE_NAMES:
        ...
        if module_fn is not None:
            native_map[module_fn] = _torch_reduce(name)
            function_map[module_fn] = _torch_grad_reduce(name)
    return native_map, function_map


NATIVE_REDUCE_MAP, TORCH_REDUCE_MAP = _build_reduce_maps()
TENSOR_REDUCE_MAP = TORCH_REDUCE_MAP
