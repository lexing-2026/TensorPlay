...
def _upcast_int_indices(index: object) -> object:
    if isinstance(index, tensorplay.Tensor):
        if index.dtype in (tensorplay.int8, tensorplay.int16, tensorplay.int32, tensorplay.uint8):
            return index.to(tensorplay.int64)
...
def _has_advanced_indexing(index: Iterable[object]) -> bool:
    ...
    return any(
        ...
        or (isinstance(idx, tensorplay.Tensor) and (idx.dtype == tensorplay.bool or idx.ndim > 0))
        for idx in index
    )
...
def _numpy_compatible_indexing(index: object) -> tuple[object, ...]:
    ...
    for idx in index:
        if isinstance(idx, int) and not isinstance(idx, bool):
            ...
            isinstance(idx, tensorplay.Tensor)
            and idx.ndim == 0
            and not tensorplay.is_floating_point(idx)
            and idx.dtype != tensorplay.bool
        ):
...
def _get_bool_depth(s: object) -> tuple[bool, int]:
    ...
    if isinstance(s, bool):
        return True, 0
    if isinstance(s, tensorplay.Tensor) and s.dtype == tensorplay.bool:
        return True, s.ndim
...
def _numpy_empty_ellipsis_patch(
    ...
    it still acts as a separator between advanced indices. This layer does not implement that behavior.
    ...
    if ellipsis_dims == 0:
        ...
        if left_advanced and right_advanced:
            # This is the case where NumPy differs from this layer
            ...
            def unsqueeze_fn(x: object) -> object:
                if isinstance(x, tensorplay.Tensor) and x.ndim >= end_ndims:
...
class ndarray:
    def __init__(self, t: tensorplay.Tensor | None = None) -> None:
        if t is None:
            self.tensor = tensorplay.Tensor()
    ...
    @normalizer
    def copy(self: ArrayLike, order: NotImplementedType = "C") -> tensorplay.Tensor:
        return self.clone()

    @normalizer
    def flatten(self: ArrayLike, order: NotImplementedType = "C") -> tensorplay.Tensor:
        return tensorplay.flatten(self)
    ...
    def fill(self, value: ArrayLike) -> None:
        # Both this layer and NumPy accept 0D arrays/tensors and scalars, and
    ...
    def __str__(self) -> str:
        return (
            str(self.tensor)
            .replace("tensor", "tensorplay.ndarray")
            .replace("dtype=tensorplay.", "dtype=")
        )
    ...
    def __eq__(self, other: object) -> ndarray:  # pyrefly: ignore[bad-override]
        try:
            ...
            # tensorplay.full accepts the python `bool` type as `dtype` at runtime;
            # the stub is too strict.
            # pyrefly: ignore[no-matching-overload]
            falsy = tensorplay.full(self.shape, fill_value=False, dtype=bool)
    ...
    def __getitem__(self, index: object) -> ndarray:
        ...
        def neg_step(i: int, s: object) -> object:
            ...
            nonlocal tensor
            tensor = tensorplay.flip(tensor, (i,))
...
def result_type(*arrays_and_dtypes: object) -> _dtypes.DType:
    tensors: list[tensorplay.Tensor] = []
    for entry in arrays_and_dtypes:
        try:
            t = asarray(entry).tensor
        except (RuntimeError, ValueError, TypeError):
            dty = _dtypes.dtype(entry)
            t = tensorplay.empty(1, dtype=dty.torch_dtype)
        tensors.append(t)
