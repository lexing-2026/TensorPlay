...
# and consume/return the framework's tensors and dtypes.
# They are also type annotated.
# Pull these functions from _funcs_impl and decorate them with @normalizer, which
# - Converts any input `np.ndarray`, `tensorplay._numpy.ndarray`, list of lists, Python scalars, etc into a `tensorplay.Tensor`.
# - Maps NumPy dtypes to framework dtypes
# - If the input to the `axis` kwarg is an ndarray, it maps it into a tuple
# - Implements the semantics for the `out=` arg
# - Wraps back the outputs into `tensorplay._numpy.ndarrays`
