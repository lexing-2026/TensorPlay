...
"""Elementwise and array-level function implementations.

Functions defined here carry numpy-compatible signatures but operate on
framework tensors.
"""
...
def atleast_1d(*arys: ArrayLike):
    res = tensorplay.atleast_1d(*arys)
...
def atleast_2d(*arys: ArrayLike):
    res = tensorplay.atleast_2d(*arys)
    if isinstance(res, tuple):
        return list(res)
    else:
        return res
...
def atleast_3d(*arys: ArrayLike):
    res = tensorplay.atleast_3d(*arys)
    if isinstance(res, tuple):
        return list(res)
    else:
        return res
...
def vstack(
    ...
    *,
    ...
    return tensorplay.vstack(tensors)
...
def hstack(
    ...
    *,
    ...
    _concat_check(tup, dtype, out=None)
    tensors = _concat_cast_helper(tup, dtype=dtype, casting=casting)
    return tensorplay.hstack(tensors)
...
def dstack(
    ...
    *,
    ...
    _concat_check(tup, dtype, out=None)
    tensors = _concat_cast_helper(tup, dtype=dtype, casting=casting)
    return tensorplay.dstack(tensors)
...
def column_stack(
    ...
    *,
    ...
    _concat_check(tup, dtype, out=None)
    tensors = _concat_cast_helper(tup, dtype=dtype, casting=casting)
    return tensorplay.column_stack(tensors)
...
def stack(
    ...
    *,
    ...
    return tensorplay.stack(tensors, dim=axis)
...
def _split_helper_int(tensor, indices_or_sections, axis, strict=False):
    ...
    return tensorplay.split(tensor, lst, axis)
...
def kron(a: ArrayLike, b: ArrayLike):
    return tensorplay.kron(a, b)


def vander(x: ArrayLike, N=None, increasing=False):
    return tensorplay.vander(x, N, increasing)
...
def linspace(
    ...
    # The endpoints are scalars by definition; pass Python floats so the
    # scalar-overload group entry (which carries requires_grad) is selected
    # instead of a tensor-end overload.
    return tensorplay.linspace(float(start), float(stop), num, dtype=dtype)
...
def geomspace(
    ...
    if axis != 0 or not endpoint:
        raise NotImplementedError
    base = tensorplay.pow(stop / start, 1.0 / (num - 1))
    logbase = tensorplay.log(base)
    return tensorplay.logspace(
        float(tensorplay.log(start) / logbase),
        float(tensorplay.log(stop) / logbase),
        num,
        base=float(base),
    )
...
def logspace(
    ...
    if dtype is None:
        dtype = _dtypes_impl.default_dtypes().float_dtype
    return tensorplay.logspace(float(start), float(stop), num, base=float(base),
                               dtype=dtype)
...
def arange(
    ...
    *,
    ...
    work_dtype = tensorplay.float64 if dtype.is_complex else dtype
    ...
    if (step > 0 and start > stop) or (step < 0 and start < stop):
        # empty range
        return tensorplay.empty(0, dtype=dtype)

    result = tensorplay.arange(start, stop, step, dtype=work_dtype)
...
def empty(
    ...
    *,
    ...
    if dtype is None:
        dtype = _dtypes_impl.default_dtypes().float_dtype
    return tensorplay.empty(shape, dtype=dtype)
...
def empty_like(
    ...
    result = tensorplay.empty_like(prototype, dtype=dtype)
...
def full(
    ...
    *,
    ...
    if not isinstance(shape, (tuple, list)):
        shape = (shape,)
    return tensorplay.full(shape, fill_value, dtype=dtype)
...
def full_like(
    ...
    result = tensorplay.full_like(a, fill_value, dtype=dtype)
    if shape is not None:
        result = result.reshape(shape)
    return result
...
def ones(
    ...
    *,
    ...
    if dtype is None:
        dtype = _dtypes_impl.default_dtypes().float_dtype
    return tensorplay.ones(shape, dtype=dtype)
...
def ones_like(
    ...
    result = tensorplay.ones_like(a, dtype=dtype)
    if shape is not None:
        result = result.reshape(shape)
    return result
...
def zeros(
    ...
    *,
    ...
    if dtype is None:
        dtype = _dtypes_impl.default_dtypes().float_dtype
    return tensorplay.zeros(shape, dtype=dtype)
...
def zeros_like(
    ...
    result = tensorplay.zeros_like(a, dtype=dtype)
    if shape is not None:
        result = result.reshape(shape)
    return result
...
def corrcoef(
    ...
    *,
    ...
    result = tensorplay.corrcoef(xy_tensor)

    if is_half:
        result = result.to(tensorplay.float16)
...
def cov(
    ...
    *,
    ...
    result = tensorplay.cov(m, correction=ddof, aweights=aweights, fweights=fweights)
...
def _conv_corr_impl(a, v, mode):
    ...
    # NumPy only accepts 1D arrays; convolution here requires 2D inputs and 3D weights
    aa = a[None, :]
    vv = v[None, None, :]
...
def bincount(x: ArrayLike, /, weights: ArrayLike | None = None, minlength=0):
    ...
    return tensorplay.bincount(x, weights, minlength)
...
def where(
    ...
    if condition.dtype != tensorplay.bool:
        condition = condition.to(tensorplay.bool)

    if x is None and y is None:
        result = tensorplay.where(condition)
    else:
        result = tensorplay.where(condition, x, y)
    return result
...
def broadcast_to(array: ArrayLike, shape, subok: NotImplementedType = False):
    return tensorplay.broadcast_to(array, size=shape)
...
def broadcast_arrays(*args: ArrayLike, subok: NotImplementedType = False):
    return tensorplay.broadcast_tensors(*args)
...
def meshgrid(*xi: ArrayLike, copy=True, sparse=False, indexing="xy"):
    ...
    if not sparse:
        ...
        output = tensorplay.broadcast_tensors(*output)
...
def indices(dimensions, dtype: DTypeLike | None = int, sparse=False):
    ...
    if sparse:
        ...
        res = tensorplay.empty((N,) + dimensions, dtype=dtype)
    for i, dim in enumerate(dimensions):
        idx = tensorplay.arange(dim, dtype=dtype).reshape(
...
def tril(m: ArrayLike, k=0):
    return tensorplay.tril(m, k)


def triu(m: ArrayLike, k=0):
    return tensorplay.triu(m, k)


def tril_indices(n, k=0, m=None):
    if m is None:
        m = n
    return tensorplay.tril_indices(n, m, offset=k)


def triu_indices(n, k=0, m=None):
    if m is None:
        m = n
    return tensorplay.triu_indices(n, m, offset=k)
...
def tril_indices_from(arr: ArrayLike, k=0):
    ...
    return tensorplay.tril_indices(arr.shape[0], arr.shape[1], offset=k)
...
def triu_indices_from(arr: ArrayLike, k=0):
    if arr.ndim != 2:
        raise ValueError("input array must be 2-d")
    # Return a tensor rather than a tuple to avoid a graphbreak
    return tensorplay.triu_indices(arr.shape[0], arr.shape[1], offset=k)
...
def tri(
    ...
    *,
    ...
    if M is None:
        M = N
    tensor = tensorplay.ones((N, M), dtype=dtype)
    return tensorplay.tril(tensor, diagonal=k)
...
def isclose(a: ArrayLike, b: ArrayLike, rtol=1.0e-5, atol=1.0e-8, equal_nan=False):
    ...
    return tensorplay.isclose(a, b, rtol=rtol, atol=atol, equal_nan=equal_nan)
...
def allclose(a: ArrayLike, b: ArrayLike, rtol=1e-05, atol=1e-08, equal_nan=False):
    ...
    a = _util.cast_if_needed(a, dtype)
    b = _util.cast_if_needed(b, dtype)
    return tensorplay.allclose(a, b, rtol=rtol, atol=atol, equal_nan=equal_nan)
...
def _tensor_equal(a1, a2, equal_nan=False):
    ...
    cond = a1 == a2
    if equal_nan:
        cond = cond | (tensorplay.isnan(a1) & tensorplay.isnan(a2))
...
def array_equiv(a1: ArrayLike, a2: ArrayLike):
    ...
    try:
        a1_t, a2_t = tensorplay.broadcast_tensors(a1, a2)
    except RuntimeError:
...
def nan_to_num(
    ...
    if x.is_complex():
        re = tensorplay.nan_to_num(x.real, nan=nan, posinf=posinf, neginf=neginf)
        im = tensorplay.nan_to_num(x.imag, nan=nan, posinf=posinf, neginf=neginf)
        return re + 1j * im
    else:
        return tensorplay.nan_to_num(x, nan=nan, posinf=posinf, neginf=neginf)
...
def take_along_axis(arr: ArrayLike, indices: ArrayLike, axis):
    ...
    return tensorplay.take_along_dim(arr, indices, axis)
...
def put_along_axis(arr: ArrayLike, indices: ArrayLike, values: ArrayLike, axis):
    ...
    indices, values = tensorplay.broadcast_tensors(indices, values)
    values = _util.cast_if_needed(values, arr.dtype)
    result = tensorplay.scatter(arr, axis, indices, values)
...
def choose(
    ...
    choices = tensorplay.stack(tensorplay.broadcast_tensors(*choices))

    # Use an analog of `gather(choices, 0, a)` which broadcasts `choices` vs `a`:
    idx_list = [
        tensorplay.arange(dim).view((1,) * i + (dim,) + (1,) * (choices.ndim - i - 1))
...
def nonzero(a: ArrayLike):
    return tensorplay.nonzero(a, as_tuple=True)


def argwhere(a: ArrayLike):
    return tensorplay.argwhere(a)


def flatnonzero(a: ArrayLike):
    return tensorplay.flatten(a).nonzero(as_tuple=True)[0]
...
def clip(
    ...
    return tensorplay.clamp(a, min, max)


def repeat(a: ArrayLike, repeats: ArrayLikeOrScalar, axis=None):
    return tensorplay.repeat_interleave(a, repeats, axis)


def tile(A: ArrayLike, reps):
    if isinstance(reps, int):
        reps = (reps,)
    return tensorplay.tile(A, reps)
...
def resize(a: ArrayLike, new_shape=None):
    ...
    if a.numel() == 0 or new_size == 0:
        ...
        return tensorplay.zeros(new_shape, dtype=a.dtype)
...
def diagonal(a: ArrayLike, offset=0, axis1=0, axis2=1):
    ...
    return tensorplay.diagonal(a, offset, axis1, axis2)
...
def trace(
    ...
    result = tensorplay.diagonal(a, offset, dim1=axis1, dim2=axis2).sum(-1, dtype=dtype)
    return result
...
def eye(
    ...
    *,
    ...
    if M is None:
        M = N
    z = tensorplay.zeros(N, M, dtype=dtype)
    z.diagonal(k).fill_(1)
    return z


def identity(n, dtype: DTypeLike | None = None, *, like: NotImplementedType = None):
    return tensorplay.eye(n, dtype=dtype)


def diag(v: ArrayLike, k=0):
    return tensorplay.diag(v, k)


def diagflat(v: ArrayLike, k=0):
    return tensorplay.diagflat(v, k)


def diag_indices(n, ndim=2):
    idx = tensorplay.arange(n)
    return (idx,) * ndim
...
def fill_diagonal(a: ArrayLike, val: ArrayLike, wrap=False):
    ...
    # tensorplay.Tensor.fill_diagonal_ only accepts scalars
    ...
    if a.ndim == 2:
        ...
        if not wrap or not tall:
            ...
            max_, min_ = a.shape
            idx = tensorplay.arange(max_ - max_ // (min_ + 1))
            mod = idx % min_
            div = idx // min_
...
def vdot(a: ArrayLike, b: ArrayLike, /):
    ...
    result = tensorplay.vdot(t_a, t_b)

    if is_half:
        result = result.to(tensorplay.float16)
    elif is_bool:
        result = result.to(tensorplay.bool)
...
def tensordot(a: ArrayLike, b: ArrayLike, axes=2):
    ...
    return tensorplay.tensordot(a, b, dims=axes)
...
def inner(a: ArrayLike, b: ArrayLike, /):
    dtype = _dtypes_impl.result_type_impl(a, b)
    is_half = dtype == tensorplay.float16 and (a.is_cpu or b.is_cpu)
    is_bool = dtype == tensorplay.bool
...
def cross(a: ArrayLike, b: ArrayLike, axisa=-1, axisb=-1, axisc=-1, axis=None):
    ...
    a = tensorplay.moveaxis(a, axisa, -1)
    b = tensorplay.moveaxis(b, axisb, -1)
    ...
    dtype = _dtypes_impl.result_type_impl(a, b)
    cp = tensorplay.empty(shape, dtype=dtype)
    ...
    return tensorplay.moveaxis(cp, -1, axisc)
...
def einsum(*operands, out=None, dtype=None, order="K", casting="safe", optimize=False):
    ...
    is_half = target_dtype == tensorplay.float16 and all(t.is_cpu for t in tensors)
    if is_half:
        target_dtype = tensorplay.float32

    is_short_int = target_dtype in [tensorplay.uint8, tensorplay.int8, tensorplay.int16, tensorplay.int32]
    if is_short_int:
        target_dtype = tensorplay.int64

    tensors = _util.typecast_tensors(tensors, target_dtype, casting)

    from tensorplay.backends import opt_einsum

    try:
        # set the global state to handle the optimize=... argument, restore on exit
        if opt_einsum.is_available():
            old_strategy = tensorplay.backends.opt_einsum.strategy
            old_enabled = tensorplay.backends.opt_einsum.enabled

            # tensorplay.einsum calls opt_einsum.contract_path, which runs into
            # https://github.com/dgasmith/opt_einsum/issues/219
            # for strategy={True, False}
            if optimize is True:
                optimize = "auto"
            elif optimize is False:
                tensorplay.backends.opt_einsum.enabled = False

            tensorplay.backends.opt_einsum.strategy = optimize
        ...
        if sublist_format:
            ...
            result = tensorplay.einsum(*operands)
        else:
            result = tensorplay.einsum(subscripts, *tensors)

    finally:
        if opt_einsum.is_available():
            tensorplay.backends.opt_einsum.strategy = old_strategy
            tensorplay.backends.opt_einsum.enabled = old_enabled
...
def sort(a: ArrayLike, axis=-1, kind=None, order: NotImplementedType = None):
    ...
    result = tensorplay.sort(a, dim=axis, stable=stable)
    return result.values


def argsort(a: ArrayLike, axis=-1, kind=None, order: NotImplementedType = None):
    a, axis, stable = _sort_helper(a, axis, kind, order)
    return tensorplay.argsort(a, dim=axis, stable=stable)
...
def searchsorted(
    ...
    return tensorplay.searchsorted(a, v, side=side, sorter=sorter)
...
def moveaxis(a: ArrayLike, source, destination):
    ...
    return tensorplay.moveaxis(a, source, destination)


def swapaxes(a: ArrayLike, axis1, axis2):
    axis1 = _util.normalize_axis_index(axis1, a.ndim)
    axis2 = _util.normalize_axis_index(axis2, a.ndim)
    return tensorplay.swapaxes(a, axis1, axis2)
...
def roll(a: ArrayLike, shift, axis=None):
    ...
    return tensorplay.roll(a, shift, axis)
...
# NB: cannot use tensorplay.reshape(a, newshape) above, because of
# (Pdb) tensorplay.reshape(tensorplay.as_tensor([1]), 1)
...
def diff(
    ...
    if prepend is not None:
        ...
        prepend = tensorplay.broadcast_to(prepend, shape)

    if append is not None:
        shape = list(a.shape)
        shape[axis] = append.shape[axis] if append.ndim > 0 else 1
        append = tensorplay.broadcast_to(append, shape)

    return tensorplay.diff(a, n, dim=axis, prepend=prepend, append=append)
...
def angle(z: ArrayLike, deg=False):
    result = tensorplay.angle(z)
    if deg:
        result = result * (180 / tensorplay.pi)
    return result


def sinc(x: ArrayLike):
    return tensorplay.sinc(x)
...
def gradient(f: ArrayLike, *varargs, axis=None, edge_order=1):
    ...
    if n == 0:
        ...
        for i, distances in enumerate(dx):
            distances = tensorplay.as_tensor(distances)
            if distances.ndim == 0:
                continue
            ...
            diffx = tensorplay.diff(distances)
    ...
    for axis, ax_dx in zip(axes, dx):
        ...
        # result allocation
        out = tensorplay.empty_like(f, dtype=otype)
...
def round(a: ArrayLike, decimals=0, out: OutArray | None = None):
    if a.is_floating_point():
        result = tensorplay.round(a, decimals=decimals)
    elif a.is_complex():
        # RuntimeError: "round_cpu" not implemented for 'ComplexFloat'
        result = tensorplay.complex(
            tensorplay.round(a.real, decimals=decimals),
            tensorplay.round(a.imag, decimals=decimals),
        )
    else:
...
def real_if_close(a: ArrayLike, tol=100):
    if not tensorplay.is_complex(a):
        return a
    if tol > 1:
        ...
        tol = tol * tensorplay.finfo(a.dtype).eps

    mask = tensorplay.abs(a.imag) < tol
    return a.real if mask.all() else a


def real(a: ArrayLike):
    return tensorplay.real(a)


def imag(a: ArrayLike):
    if a.is_complex():
        return a.imag
    return tensorplay.zeros_like(a)


def iscomplex(x: ArrayLike):
    if tensorplay.is_complex(x):
        return x.imag != 0
    return tensorplay.zeros_like(x, dtype=tensorplay.bool)


def isreal(x: ArrayLike):
    if tensorplay.is_complex(x):
        return x.imag == 0
    return tensorplay.ones_like(x, dtype=tensorplay.bool)


def iscomplexobj(x: ArrayLike):
    return tensorplay.is_complex(x)


def isrealobj(x: ArrayLike):
    return not tensorplay.is_complex(x)


def isneginf(x: ArrayLike, out: OutArray | None = None):
    return tensorplay.isneginf(x)


def isposinf(x: ArrayLike, out: OutArray | None = None):
    return tensorplay.isposinf(x)


def i0(x: ArrayLike):
    return tensorplay.special.i0(x)
...
def hamming(M):
    dtype = _dtypes_impl.default_dtypes().float_dtype
    return tensorplay.hamming_window(M, periodic=False, dtype=dtype)


def hanning(M):
    dtype = _dtypes_impl.default_dtypes().float_dtype
    return tensorplay.hann_window(M, periodic=False, dtype=dtype)


def kaiser(M, beta):
    dtype = _dtypes_impl.default_dtypes().float_dtype
    return tensorplay.kaiser_window(M, beta=beta, periodic=False, dtype=dtype)


def blackman(M):
    dtype = _dtypes_impl.default_dtypes().float_dtype
    return tensorplay.blackman_window(M, periodic=False, dtype=dtype)


def bartlett(M):
    dtype = _dtypes_impl.default_dtypes().float_dtype
    return tensorplay.bartlett_window(M, periodic=False, dtype=dtype)
...
array_type = [
    [tensorplay.float16, tensorplay.float32, tensorplay.float64],
    [None, tensorplay.complex64, tensorplay.complex128],
]
array_precision = {
    tensorplay.float16: 0,
    tensorplay.float32: 1,
    tensorplay.float64: 2,
    tensorplay.complex64: 1,
    tensorplay.complex128: 2,
}
...
def histogram(
    ...
    if range is None:
        h, b = tensorplay.histogram(a, bins, weight=weights, density=bool(density))
    else:
        h, b = tensorplay.histogram(
...
def histogramdd(
    ...
    sample = tensorplay.atleast_2d(sample)
    ...
    if weights is not None:
        ...
        range = tensorplay.cat(mm).reshape(2, -1).T.flatten()
    ...
    h, b = tensorplay.histogramdd(sample, bins, range, density=bool(density), **w_kwd)
...
def min_scalar_type(a: ArrayLike, /):
    ...
    if a.dtype == tensorplay.bool:
        ...
        fi = tensorplay.finfo(tensorplay.float32)
        fits_in_single = a.dtype == tensorplay.complex64 or (
            fi.min <= a.real <= fi.max and fi.min <= a.imag <= fi.max
        )
        dtype = tensorplay.complex64 if fits_in_single else tensorplay.complex128

    elif a.dtype.is_floating_point:
        for dt in [tensorplay.float16, tensorplay.float32, tensorplay.float64]:
            fi = tensorplay.finfo(dt)
            if fi.min <= a <= fi.max:
                dtype = dt
                break
    else:
        # must be integer
        for dt in [tensorplay.uint8, tensorplay.int8, tensorplay.int16, tensorplay.int32, tensorplay.int64]:
            # Prefer unsigned int where possible, as numpy does.
            ii = tensorplay.iinfo(dt)
            if ii.min <= a <= ii.max:
                dtype = dt
                break
...
def pad(array: ArrayLike, pad_width: ArrayLike, mode="constant", **kwargs):
    ...
    # `value` must be a python scalar for tensorplay.nn.functional.pad
    typ = _dtypes_impl.python_type_for_torch(array.dtype)
    value = typ(value)

    pad_width = tensorplay.broadcast_to(pad_width, (array.ndim, 2))
    pad_width = tensorplay.flip(pad_width, (0,)).flatten()

    return tensorplay.nn.functional.pad(array, tuple(pad_width), value=value)
