"""Define analogs of numpy dtypes backed by framework dtypes.
Define the scalar types and supported dtypes with numpy <-> framework mappings.
"""
...
class generic:
    ...
    def __new__(cls, value: object) -> ndarray:
        ...
        if isinstance(value, str) and value in ["inf", "nan"]:
            value = {"inf": tensorplay.inf, "nan": tensorplay.nan}[value]
...
class int8(signedinteger):
    name = "int8"
    typecode = "b"
    torch_dtype = tensorplay.int8


class int16(signedinteger):
    name = "int16"
    typecode = "h"
    torch_dtype = tensorplay.int16


class int32(signedinteger):
    name = "int32"
    typecode = "i"
    torch_dtype = tensorplay.int32


class int64(signedinteger):
    name = "int64"
    typecode = "l"
    torch_dtype = tensorplay.int64
...
class uint8(unsignedinteger):
    name = "uint8"
    typecode = "B"
    torch_dtype = tensorplay.uint8


class uint16(unsignedinteger):
    name = "uint16"
    typecode = "H"
    torch_dtype = tensorplay.uint16


class uint32(signedinteger):
    name = "uint32"
    typecode = "I"
    torch_dtype = tensorplay.uint32


class uint64(signedinteger):
    name = "uint64"
    typecode = "L"
    torch_dtype = tensorplay.uint64
...
class float16(floating):
    name = "float16"
    typecode = "e"
    torch_dtype = tensorplay.float16


class float32(floating):
    name = "float32"
    typecode = "f"
    torch_dtype = tensorplay.float32


class float64(floating):
    name = "float64"
    typecode = "d"
    torch_dtype = tensorplay.float64


class complex64(complexfloating):
    name = "complex64"
    typecode = "F"
    torch_dtype = tensorplay.complex64


class complex128(complexfloating):
    name = "complex128"
    typecode = "D"
    torch_dtype = tensorplay.complex128


class bool_(generic):
    name = "bool_"
    typecode = "?"
    torch_dtype = tensorplay.bool
...
def sctype_from_torch_dtype(torch_dtype: tensorplay.dtype) -> type[generic]:
...
class DType:
    ...
    def __init__(self, arg: object) -> None:
        # a framework dtype object?
        if isinstance(arg, tensorplay.dtype):
            sctype = _torch_dtypes[arg]
        elif isinstance(arg, tensorplay.Tensor):
    ...
    @property
    def torch_dtype(self) -> tensorplay.dtype:
...
def set_default_dtype(
    ...
    ----------
    fp_dtype
        Allowed values are "numpy", "framework" or dtype_like things which
    ...
    ``complex_dtypes`` and ``int_dtype``. These attributes store framework
    dtypes.
    ...
    """
    if fp_dtype not in ["numpy", "framework"]:
        fp_dtype = dtype(fp_dtype).torch_dtype
    if int_dtype not in ["numpy", "framework"]:
        int_dtype = dtype(int_dtype).torch_dtype

    if fp_dtype == "numpy":
        float_dtype = tensorplay.float64
    elif fp_dtype == "framework":
        float_dtype = tensorplay.float32
    else:
    ...
    if int_dtype in ["numpy", "framework"]:
        int_dtype = tensorplay.int64
