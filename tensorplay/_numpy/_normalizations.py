"""Normalize arguments: convert array_likes to tensors, dtypes to framework dtypes and so on."""
...
# *after* normalization runs (e.g. an ArrayLike argument is a tensorplay.Tensor by the
# time the implementer body sees it).
ArrayLike: typing.TypeAlias = tensorplay.Tensor
Scalar = int | float | complex | bool
ArrayLikeOrScalar = ArrayLike | Scalar

# DTypeLike normalizes to a tensorplay.dtype (or None when no dtype is given).
DTypeLike: typing.TypeAlias = "tensorplay.dtype | None"
...
NDArray: typing.TypeAlias = tensorplay.Tensor
...
# the argument equals the parameter's default (the NumPy default, so
...
def normalize_outarray(
    ...
    # A traced ``out=`` argument may arrive as a bare tensor;
...
def maybe_copy_to(
    ...
    if out is None:
        return result
    elif isinstance(result, tensorplay.Tensor):
...
def wrap_tensors(result: object) -> object:
    ...
    if isinstance(result, tensorplay.Tensor):
