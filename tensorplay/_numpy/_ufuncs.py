...
def _ufunc_postprocess(
    result: tensorplay.Tensor, out: ndarray | None, casting: CastingModes
) -> tensorplay.Tensor:
    if out is not None:
        result = _util.typecast_tensor(result, out.dtype.torch_dtype, casting)
        result = tensorplay.broadcast_to(result, out.shape)
    return result
...
_binary = [
    ...
    if not name.startswith("_") and name not in ["tensorplay", "matmul", "divmod", "ldexp"]
]
...
def deco_binary_ufunc(
    torch_func: Callable[..., tensorplay.Tensor],
) -> Callable[..., object]:
    """Common infra for binary ufuncs.

    Normalize arguments, sort out type casting, broadcasting and delegate to
    the framework's functions for the actual work.
    """
    ...
    def wrapped(
        ...
        *,
        ...
        if dtype is not None:
            ...
            def cast(
                x: tensorplay.Tensor | bool | int | float | complex, dtype: tensorplay.dtype
            ) -> tensorplay.Tensor:
                if isinstance(x, tensorplay.Tensor):
                    return _util.typecast_tensor(x, dtype, casting)
                else:
                    return tensorplay.as_tensor(x, dtype=dtype)

            x1 = cast(x1, dtype)
            x2 = cast(x2, dtype)
        elif isinstance(x1, tensorplay.Tensor) and isinstance(x2, tensorplay.Tensor):
...
def ldexp(
    ...
    *,
    ...
    if dtype is not None:
        if isinstance(x1, tensorplay.Tensor):
            x1 = _util.typecast_tensor(x1, dtype, casting)
        else:
            x1 = tensorplay.as_tensor(x1, dtype=dtype)
    else:
        if not isinstance(x1, tensorplay.Tensor):
            x1 = tensorplay.as_tensor(x1)
            x1 = _util.cast_int_to_float(x1)

    x2 = tensorplay.as_tensor(x2)
    ...
    if x1.dtype == tensorplay.float16:
        # tensorplay.ldexp(f16, int) -> f32, undo it
...
_unary = [
    ...
    if not name.startswith("_") and name != "tensorplay"
]
