...
import tensorplay
from tensorplay import DeviceType


_DEVICE_TYPES = tuple(
    value
    for value in (
        getattr(DeviceType, "CUDA", None),
        getattr(DeviceType, "XPU", None),
        getattr(DeviceType, "MTIA", None),
        getattr(DeviceType, "HPU", None),
        getattr(DeviceType, "PrivateUse1", None),
    )
    if value is not None
)


def _coerce_event(event, event_id=None):
    if isinstance(event, (FunctionEvent, FunctionEventAvg)):
        return event
    if not isinstance(event, (tuple, list)) or len(event) < 4:
        return event
    values = list(event) + [None] * max(0, 13 - len(event))
    name, kind, start_ns, end_ns, thread, shapes, dtypes, _site, gpu_ms, out_bytes, stack, kernels, flops = values[:13]
    function_event = FunctionEvent(
        id=len(event) if event_id is None else event_id,
        name=str(name),
        thread=int(thread) if thread is not None else 0,
        start_us=float(start_ns) / 1000.0,
        end_us=float(end_ns) / 1000.0,
        input_shapes=shapes,
        input_dtypes=dtypes,
        stack=stack or [],
        trace_name=str(name),
        use_device="cuda" if gpu_ms is not None and gpu_ms >= 0 else None,
        cpu_memory_usage=int(out_bytes or 0),
        device_type=DeviceType.CPU,
        flops=flops,
        scope=1 if kind == "b" else 0,
        is_user_annotation=kind == "u",
    )
    if gpu_ms is not None and gpu_ms >= 0:
        function_event.append_kernel("device", "cuda", float(gpu_ms) * 1000.0)
...
class EventList(list):
    ...
    Example::
        ...
        with profile(activities=[ProfilerActivity.CPU]) as prof:
            x = tensorplay.randn(100, 100)
            y = tensorplay.matmul(x, x)
    ...
    def __init__(self, *args, **kwargs):
        ...
        kwargs.pop("base_ns", None)
        kwargs.pop("gpu_activities", None)
        kwargs.pop("mem_events", None)
        kwargs.pop("samples", None)
        kwargs.pop("with_stack", None)
        # pyrefly: ignore [not-iterable]
        super().__init__(
            [
                _coerce_event(event, event_id=index)
                for index, event in enumerate(args[0] if args else ())
            ],
            **kwargs,
        )
    ...
    def append(self, value):
        super().append(_coerce_event(value, event_id=len(self)))
        self._tree_built = False

    def extend(self, values):
        start = len(self)
        super().extend(
            _coerce_event(value, event_id=start + index)
            for index, value in enumerate(values)
        )
        self._tree_built = False

    def insert(self, index, value):
        super().insert(index, _coerce_event(value, event_id=index))
        self._tree_built = False
    ...
    def export_chrome_trace(self, path, **_kwargs):
        ...
        with open(path, "w") as f:
            ...
            for evt in self:
                ...
                for _ in evt.kernels:
                    ...
                    # Note: use tensorplay.profiler to get device kernel trace
                    next_id += 1
            if len(self) > 0:
    ...
    def key_averages(
        ...
        Args:
            ...
            and tensorplay::add.out will be aggregated separately
        ...
        """
        if group_by_input_shape is not None:
            group_by_input_shapes = group_by_input_shape
        ...
        for evt in avg_list:
            evt.stack = (evt.stack or [])[:group_by_stack_n]
...
class FunctionEvent(FormattedTimesMixin):
    ...
    See Also:
        - :class:`tensorplay.profiler.profile`: Context manager for profiling
...
class StringTable(defaultdict):
    def __missing__(self, key):
        ...
        # the short sequences
        demangle = getattr(tensorplay._C, "_demangle", None)
        self[key] = demangle(key) if demangle is not None and len(key) > 1 else key
        return self[key]
