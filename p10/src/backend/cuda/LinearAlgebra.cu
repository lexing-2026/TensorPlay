...
Tensor linalg_cond_native_cuda(const Tensor& self,
                               const std::optional<Scalar>& ord) {
    return linalg_cond_impl(self, ord);
}

Tensor& linalg_cond_native_cuda_out(const Tensor& self,
                                    const std::optional<Scalar>& ord, Tensor& out) {
    return write_linalg_norm_output(linalg_cond_impl(self, ord), out);
}

Tensor linalg_cond_string_native_cuda(const Tensor& self, const std::string& ord) {
    return linalg_cond_string_impl(self, ord);
}

Tensor& linalg_cond_string_native_cuda_out(const Tensor& self,
                                           const std::string& ord, Tensor& out) {
    return write_linalg_norm_output(linalg_cond_string_impl(self, ord), out);
}

Tensor linalg_matrix_sqrth_native_cuda(const Tensor& self) {
...
TENSORPLAY_LIBRARY_IMPL(CUDA, NativeLinearAlgebra) {
    ...
    m.impl("linalg_cond", linalg_cond_native_cuda);
    m.impl("linalg_cond.out", linalg_cond_native_cuda_out);
    m.impl("linalg_cond.p_str", linalg_cond_string_native_cuda);
    m.impl("linalg_cond.p_str_out", linalg_cond_string_native_cuda_out);
    m.impl("linalg_matrix_sqrth", linalg_matrix_sqrth_native_cuda);
