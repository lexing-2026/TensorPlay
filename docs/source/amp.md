...
use lower precision floating point datatype (`lower_precision_fp`): `tensorplay.float16` (`half`) or `tensorplay.bfloat16`. Some ops, like linear layers and convolutions,
...
`tensorplay._functorch.config.backward_pass_autocast` setting is
...
These ops always go through autocasting whether they are invoked as part of a {class}`tensorplay.nn.Module`,
as a function, or as a {class}`tensorplay.Tensor` method. If functions are exposed in multiple namespaces,
...
The backward passes of {func}`tensorplay.nn.functional.binary_cross_entropy` (and {mod}`tensorplay.nn.BCELoss`, which wraps it)
...
In this case, combine the two layers using {func}`tensorplay.nn.functional.binary_cross_entropy_with_logits`
or {mod}`tensorplay.nn.BCEWithLogitsLoss`. `binary_cross_entropy_with_logits` and `BCEWithLogits`
