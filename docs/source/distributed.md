...
TensorPlay distributed package supports Linux (stable), macOS (stable), and Windows (prototype).
By default for Linux, the Gloo and NCCL backends are built and included in TensorPlay
distributed (NCCL only when building with CUDA). MPI is an optional backend that can only be
included if you build TensorPlay from source. (e.g. building TensorPlay on a host that has MPI
installed.)
:::{note}
...
machines. The class {func}`tensorplay.nn.parallel.DistributedDataParallel` builds on this
...
{doc}`multiprocessing` and {func}`tensorplay.nn.DataParallel` in that it supports
...
{func}`tensorplay.nn.parallel.DistributedDataParallel` wrapper may still have advantages over other
approaches to data-parallelism, including {func}`tensorplay.nn.DataParallel`:
...
The package needs to be initialized using the {func}`tensorplay.distributed.init_process_group`
or {func}`tensorplay.distributed.device_mesh.init_device_mesh` function before calling any other methods.
...
Once {func}`tensorplay.distributed.init_process_group` was run, the following functions can be used. To
check whether the process group has already been initialized use {func}`tensorplay.distributed.is_initialized`.
...
processes using some means other than tensorplay.distributed primitives \_after\_ calling destroy and
...
into play. {func}`~tensorplay.distributed.new_group` function can be
used to create new groups, with arbitrary subsets of all processes. It returns
an opaque group handle that can be given as a `group` argument to all collectives
(collectives are distributed functions to exchange information in certain well-known programming patterns).
```{eval-rst}
.. autodata:: tensorplay.distributed.distributed_core.GroupMember
```
...
distributed process group easily. {func}`~tensorplay.distributed.device_mesh.init_device_mesh` function can be
used to create new DeviceMesh, with a mesh shape describing the device topology.

## Point-to-point communication

{func}`~tensorplay.distributed.isend` and {func}`~tensorplay.distributed.irecv`
...
- `get_future()` - returns `tensorplay._C.Future` object. Supported for NCCL, also supported for most operations on GLOO
...
.. class:: reduce_op
    ...
    :class:`~tensorplay.distributed.ReduceOp` is recommended to use instead.
...
{func}`tensorplay.distributed.init_process_group` (by explicitly creating the store
as an alternative to specifying `init_method`.) There are 3 choices for
Key-Value Stores: {class}`~tensorplay.distributed.TCPStore`,
{class}`~tensorplay.distributed.FileStore`, and {class}`~tensorplay.distributed.HashStore`.
...
deprecated. As of today, TensorPlay Distributed's preferred programming model
...
please contact TensorPlay Distributed's maintainers.
:::
(object_collectives)=
...
when imported.
When manually importing this backend and invoking {func}`tensorplay.distributed.init_process_group`
...
function in {func}`tensorplay.multiprocessing.spawn`. This helper function
...
implementation](https://github.com/tensorplay/examples/tree/master/imagenet)
...
2. Ensures all other ranks stop, by using a `tensorplay.distributed.barrier()` that will release once the debugged rank issues a `continue`
3. Reroutes stdin from the child process such that it connects to your terminal.
To use it, simply issue `tensorplay.distributed.breakpoint(rank)` on all ranks, using the same value for `rank` in each case.
...
**Prerequisites**
Install required dependencies (not bundled with TensorPlay by default):
```bash
...
+-------------------+------------------------------------------+
| ``MASTER_PORT``   | TCP port used by the TCPStore master.    |
+-------------------+------------------------------------------+
```
...
{class}`~tensorplay.distributed.debug._frontend.DebugHandler`:
```python
...
`StacksHandler <https://github.com/tensorplay/tensorplay/blob/main/tensorplay/distributed/debug/_debug_handlers.py>`_
in ``tensorplay/distributed/debug/_debug_handlers.py``.
...
```{eval-rst}
.. py:module:: tensorplay.distributed.algorithms
```
```{eval-rst}
.. py:module:: tensorplay.distributed.algorithms.ddp_comm_hooks
```
```{eval-rst}
.. py:module:: tensorplay.distributed.algorithms.model_averaging
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.utils
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.utils.data
```
```{eval-rst}
.. py:module:: tensorplay.distributed.launcher
```
```{eval-rst}
.. py:module:: tensorplay.distributed.nn
```
```{eval-rst}
.. py:module:: tensorplay.distributed.nn.api
```
```{eval-rst}
.. py:module:: tensorplay.distributed.nn.jit
```
```{eval-rst}
.. py:module:: tensorplay.distributed.nn.jit.templates
```
```{eval-rst}
.. py:module:: tensorplay.distributed.algorithms.ddp_comm_hooks.ddp_zero_hook
```
```{eval-rst}
.. py:module:: tensorplay.distributed.algorithms.ddp_comm_hooks.debugging_hooks
```
```{eval-rst}
.. py:module:: tensorplay.distributed.algorithms.ddp_comm_hooks.default_hooks
```
```{eval-rst}
.. py:module:: tensorplay.distributed.algorithms.ddp_comm_hooks.mixed_precision_hooks
```
```{eval-rst}
.. py:module:: tensorplay.distributed.algorithms.ddp_comm_hooks.optimizer_overlap_hooks
```
```{eval-rst}
.. py:module:: tensorplay.distributed.algorithms.ddp_comm_hooks.post_localSGD_hook
```
```{eval-rst}
.. py:module:: tensorplay.distributed.algorithms.ddp_comm_hooks.powerSGD_hook
```
```{eval-rst}
.. py:module:: tensorplay.distributed.algorithms.ddp_comm_hooks.quantization_hooks
```
```{eval-rst}
.. py:module:: tensorplay.distributed.algorithms.join
```
```{eval-rst}
.. py:module:: tensorplay.distributed.algorithms.model_averaging.averagers
```
```{eval-rst}
.. py:module:: tensorplay.distributed.algorithms.model_averaging.hierarchical_model_averager
```
```{eval-rst}
.. py:module:: tensorplay.distributed.algorithms.model_averaging.utils
```
```{eval-rst}
.. py:module:: tensorplay.distributed.argparse_util
```
```{eval-rst}
.. py:module:: tensorplay.distributed.p10d_logger
```
```{eval-rst}
.. py:module:: tensorplay.distributed.checkpoint.api
```
```{eval-rst}
.. py:module:: tensorplay.distributed.checkpoint.default_planner
```
```{eval-rst}
.. py:module:: tensorplay.distributed.checkpoint.filesystem
```
```{eval-rst}
.. py:module:: tensorplay.distributed.checkpoint.hf_storage
```
```{eval-rst}
.. py:module:: tensorplay.distributed.checkpoint.quantized_hf_storage
```
```{eval-rst}
.. py:module:: tensorplay.distributed.checkpoint.metadata
```
```{eval-rst}
.. py:module:: tensorplay.distributed.checkpoint.optimizer
```
```{eval-rst}
.. py:module:: tensorplay.distributed.checkpoint.planner
```
```{eval-rst}
.. py:module:: tensorplay.distributed.checkpoint.planner_helpers
```
```{eval-rst}
.. py:module:: tensorplay.distributed.checkpoint.resharding
```
```{eval-rst}
.. py:module:: tensorplay.distributed.checkpoint.state_dict_loader
```
```{eval-rst}
.. py:module:: tensorplay.distributed.checkpoint.state_dict_saver
```
```{eval-rst}
.. py:module:: tensorplay.distributed.checkpoint.stateful
```
```{eval-rst}
.. py:module:: tensorplay.distributed.checkpoint.storage
```
```{eval-rst}
.. py:module:: tensorplay.distributed.checkpoint.utils
```
```{eval-rst}
.. py:module:: tensorplay.distributed.constants
```
```{eval-rst}
.. py:module:: tensorplay.distributed.device_mesh
```
```{eval-rst}
.. py:module:: tensorplay.distributed.distributed_core
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.agent.server.api
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.agent.server.local_elastic_agent
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.events.api
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.events.handlers
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.metrics.api
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.multiprocessing.api
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.multiprocessing.errors.error_handler
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.multiprocessing.errors.handlers
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.multiprocessing.redirects
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.multiprocessing.tail_log
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.rendezvous.api
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.rendezvous.c10d_rendezvous_backend
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.rendezvous.dynamic_rendezvous
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.rendezvous.etcd_rendezvous
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.rendezvous.etcd_rendezvous_backend
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.rendezvous.etcd_server
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.rendezvous.etcd_store
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.rendezvous.static_tcp_rendezvous
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.rendezvous.utils
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.timer.api
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.timer.file_based_local_timer
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.timer.local_timer
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.utils.api
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.utils.data.cycling_iterator
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.utils.data.elastic_distributed_sampler
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.utils.distributed
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.utils.log_level
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.utils.logging
```
```{eval-rst}
.. py:module:: tensorplay.distributed.elastic.utils.store
```
```{eval-rst}
.. py:module:: tensorplay.distributed.fsdp.api
```
```{eval-rst}
.. py:module:: tensorplay.distributed.fsdp.fully_sharded_data_parallel
```
```{eval-rst}
.. py:module:: tensorplay.distributed.fsdp.sharded_grad_scaler
```
```{eval-rst}
.. py:module:: tensorplay.distributed.fsdp.wrap
```
```{eval-rst}
.. py:module:: tensorplay.distributed.logging_handlers
```
```{eval-rst}
.. py:module:: tensorplay.distributed.nn.api.remote_module
```
```{eval-rst}
.. py:module:: tensorplay.distributed.nn.functional
```
```{eval-rst}
.. py:module:: tensorplay.distributed.nn.jit.instantiator
```
```{eval-rst}
.. py:module:: tensorplay.distributed.nn.jit.templates.remote_module_template
```
```{eval-rst}
.. py:module:: tensorplay.distributed.optim.apply_optimizer_in_backward
```
```{eval-rst}
.. py:module:: tensorplay.distributed.optim.functional_adadelta
```
```{eval-rst}
.. py:module:: tensorplay.distributed.optim.functional_adagrad
```
```{eval-rst}
.. py:module:: tensorplay.distributed.optim.functional_adam
```
```{eval-rst}
.. py:module:: tensorplay.distributed.optim.functional_adamax
```
```{eval-rst}
.. py:module:: tensorplay.distributed.optim.functional_adamw
```
```{eval-rst}
.. py:module:: tensorplay.distributed.optim.functional_rmsprop
```
```{eval-rst}
.. py:module:: tensorplay.distributed.optim.functional_rprop
```
```{eval-rst}
.. py:module:: tensorplay.distributed.optim.functional_sgd
```
```{eval-rst}
.. py:module:: tensorplay.distributed.optim.named_optimizer
```
```{eval-rst}
.. py:module:: tensorplay.distributed.optim.optimizer
```
```{eval-rst}
.. py:module:: tensorplay.distributed.optim.post_localSGD_optimizer
```
```{eval-rst}
.. py:module:: tensorplay.distributed.optim.utils
```
```{eval-rst}
.. py:module:: tensorplay.distributed.optim.zero_redundancy_optimizer
```
```{eval-rst}
.. py:module:: tensorplay.distributed.remote_device
```
```{eval-rst}
.. py:module:: tensorplay.distributed.rendezvous
```
```{eval-rst}
.. py:module:: tensorplay.distributed.rpc.api
```
```{eval-rst}
.. py:module:: tensorplay.distributed.rpc.backend_registry
```
```{eval-rst}
.. py:module:: tensorplay.distributed.rpc.constants
```
```{eval-rst}
.. py:module:: tensorplay.distributed.rpc.functions
```
```{eval-rst}
.. py:module:: tensorplay.distributed.rpc.internal
```
```{eval-rst}
.. py:module:: tensorplay.distributed.rpc.options
```
```{eval-rst}
.. py:module:: tensorplay.distributed.rpc.rref_proxy
```
```{eval-rst}
.. py:module:: tensorplay.distributed.rpc.server_process_global_profiler
```
```{eval-rst}
.. py:module:: tensorplay.distributed.tensor.parallel.api
```
```{eval-rst}
.. py:module:: tensorplay.distributed.tensor.parallel.ddp
```
```{eval-rst}
.. py:module:: tensorplay.distributed.tensor.parallel.fsdp
```
```{eval-rst}
.. py:module:: tensorplay.distributed.tensor.parallel.input_reshard
```
```{eval-rst}
.. py:module:: tensorplay.distributed.tensor.parallel.loss
```
```{eval-rst}
.. py:module:: tensorplay.distributed.tensor.parallel.style
```
```{eval-rst}
.. py:module:: tensorplay.distributed.utils
```
```{eval-rst}
.. py:module:: tensorplay.distributed.checkpoint.state_dict
```
