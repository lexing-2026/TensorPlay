...
GPU.  Since we often deal with large amounts of data in TensorPlay,
...
to [blow through your memory](https://github.com/tensorplay/tensorplay/issues/958)
...
TensorPlay uses a caching memory allocator to speed up memory allocations. As a
...
{class}`tensorplay.utils.data.DataLoader`'s documentation for how to
...
{class}`~tensorplay.nn.Module` with {class}`~tensorplay.nn.DataParallel` or
{func}`~tensorplay.nn.parallel.data_parallel`. Input to each the {meth}`forward` on
each device will only be part of the entire input. Because the unpack operation
{func}`tensorplay.nn.utils.rnn.pad_packed_sequence` by default only pads up to the
...
{func}`~tensorplay.nn.utils.rnn.pad_packed_sequence` to make sure that the
