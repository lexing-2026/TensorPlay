...
```{currentmodule} tensorplay.amp

```

Ordinarily, "automatic mixed precision training" means training with
{class}`tensorplay.autocast` and {class}`tensorplay.amp.GradScaler` together.

Instances of {class}`tensorplay.autocast` enable autocasting for chosen regions.
Autocasting automatically chooses the precision for operations to improve performance
while maintaining accuracy.

Instances of {class}`tensorplay.amp.GradScaler` help perform the steps of
...
{class}`tensorplay.autocast` and {class}`tensorplay.amp.GradScaler` are modular.
...
for epoch in epochs:
    for input, target in data:
        ...
        with autocast(device_type='cuda', dtype=tensorplay.float16):
            output = model(input)
...
(see {func}`tensorplay.nn.utils.clip_grad_norm_`) or maximum magnitude (see {func}`tensorplay.nn.utils.clip_grad_value_`)
...
```{currentmodule} tensorplay.amp.GradScaler
...
{func}`tensorplay.autograd.grad`, combines them to create the penalty value,
...
for epoch in epochs:
    for input, target in data:
        ...
        # Creates gradients
        grad_params = tensorplay.autograd.grad(outputs=loss,
...
passed to {func}`tensorplay.autograd.grad` should be scaled. The resulting gradients
...
for epoch in epochs:
    for input, target in data:
        ...
        scaled_grad_params = tensorplay.autograd.grad(outputs=scaler.scale(loss),
                                                 inputs=model.parameters(),
                                                 create_graph=True)
...
```python
scaler = tensorplay.amp.GradScaler()
...
Even if {class}`tensorplay.nn.DataParallel` spawns threads to run the forward pass on each device.
...
{class}`tensorplay.nn.parallel.DistributedDataParallel`'s documentation recommends one GPU per process for best
...
Here {class}`tensorplay.nn.parallel.DistributedDataParallel` may spawn a side thread to run the forward pass on each
device, like {class}`tensorplay.nn.DataParallel`. {ref}`The fix is the same <amp-dataparallel>`:
...
(subclasses of {class}`tensorplay.autograd.Function`), changes are required for
...
with autocast(device_type='cuda', dtype=tensorplay.float16):
    ...
    with autocast(device_type='cuda', dtype=tensorplay.float16, enabled=False):
        output = imported_function(input1.float(), input2.float())
```

If you're the function's author (or can alter its definition) a better solution is to use the
{func}`tensorplay.amp.custom_fwd` and {func}`tensorplay.amp.custom_bwd` decorators as shown in
...
```python
class MyMM(tensorplay.autograd.Function):
    @staticmethod
    @custom_fwd
    def forward(ctx, a, b):
...
Consider a custom function that requires `tensorplay.float32` inputs.
Apply {func}`custom_fwd(device_type='cuda', cast_inputs=tensorplay.float32) <custom_fwd>` to `forward`
...
```python
class MyFloat32Func(tensorplay.autograd.Function):
    @staticmethod
    @custom_fwd(device_type='cuda', cast_inputs=tensorplay.float32)
