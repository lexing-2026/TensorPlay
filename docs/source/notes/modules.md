...
TensorPlay uses modules to represent neural networks. Modules are:
...
This note describes modules, and is intended for all TensorPlay users. Since modules are so fundamental to TensorPlay,
...
To get started, let's look at a simpler, custom version of TensorPlay's {class}`~tensorplay.nn.Linear` module.
...
class MyLinear(nn.Module):
  def __init__(self, in_features, out_features):
    super().__init__()
    self.weight = nn.Parameter(tensorplay.randn(in_features, out_features))
    self.bias = nn.Parameter(tensorplay.randn(out_features))
...
- **It inherits from the base Module class.**
  All modules should subclass {class}`~tensorplay.nn.Module` for composability with other modules.
- **It defines some "state" that is used in computation.**
  ...
  transformation. Because each of these is defined as a {class}`~tensorplay.nn.parameter.Parameter`, they are
  *registered* for the module and will automatically be tracked and returned from calls
  to {func}`~tensorplay.nn.Module.parameters`. Parameters can be
...
```{code-block} python
m = MyLinear(4, 3)
sample_input = tensorplay.randn(4)
m(sample_input)
...
descent methods. TensorPlay's autograd system automatically takes care of this backward pass computation, so it
...
{func}`~tensorplay.nn.Module.parameters` or {func}`~tensorplay.nn.Module.named_parameters`,
...
"learned". A later section of this note shows how to update these parameters using one of TensorPlay's Optimizers.
...
The simplest way to do this is using the {class}`~tensorplay.nn.Sequential` module. It allows us to chain together
multiple modules:
...
Note that {class}`~tensorplay.nn.Sequential` automatically feeds the output of the first `MyLinear` module as input
into the {class}`~tensorplay.nn.ReLU`, and the output of that as input into the second `MyLinear` module. As
...
children of a module can be iterated through via a call to {func}`~tensorplay.nn.Module.children` or
{func}`~tensorplay.nn.Module.named_children`:
...
To go deeper than just the immediate children, {func}`~tensorplay.nn.Module.modules` and
{func}`~tensorplay.nn.Module.named_modules` *recursively* iterate through a module and its child modules:
...
The {class}`~tensorplay.nn.ModuleList` and {class}`~tensorplay.nn.ModuleDict` modules are useful here; they
...
This means that calls to {func}`~tensorplay.nn.Module.parameters` and {func}`~tensorplay.nn.Module.named_parameters` will
...
dynamic_net.to(dtype=tensorplay.float64)

dynamic_net(tensorplay.randn(5, device='cuda', dtype=tensorplay.float64))
: tensor([6.5166], device='cuda:0', dtype=tensorplay.float64, grad_fn=<AddBackward0>)
```

More generally, an arbitrary function can be applied to a module and its submodules recursively by
using the {func}`~tensorplay.nn.Module.apply` function. For example, to apply custom initialization to parameters
...
manipulated. To allow for quick and easy construction of neural networks with minimal boilerplate, TensorPlay
provides a large library of performant modules within the {mod}`tensorplay.nn` namespace that perform common neural
...
Once a network is built, it has to be trained, and its parameters can be easily optimized with one of TensorPlay’s
Optimizers from {mod}`tensorplay.optim`:

```{code-block} python
# Create the network (from previous section) and optimizer
net = Net()
optimizer = tensorplay.optim.SGD(net.parameters(), lr=1e-4, weight_decay=1e-2, momentum=0.9)

# Run a sample training loop that "teaches" the network
# to output the constant zero function
for _ in range(10000):
  input = tensorplay.randn(4)
  output = net(input)
  loss = tensorplay.abs(output)
  net.zero_grad()
  loss.backward()
  optimizer.step()
...
to its absolute value by employing {func}`tensorplay.abs` as a loss function. While this is not a very interesting task, the
...
training mode and can be switched between training and evaluation modes using {func}`~tensorplay.nn.Module.train` and
{func}`~tensorplay.nn.Module.eval`. They can behave differently depending on which mode they are in. For example, the
{class}`~tensorplay.nn.BatchNorm` module maintains a running mean and variance during training that are not updated
...
```{code-block} python
# Save the module
tensorplay.save(net.state_dict(), 'net.pt')

...

# Load the module later on
new_net = Net()
new_net.load_state_dict(tensorplay.load('net.pt'))
...
computation but is not learnable. For such cases, TensorPlay provides the concept of "buffers", both "persistent"
...
This snippet shows how to use {func}`~tensorplay.nn.Module.register_buffer` to accomplish this:
...
class RunningMean(nn.Module):
  def __init__(self, num_features, momentum=0.9):
    ...
    self.register_buffer('mean', tensorplay.zeros(num_features))
  def forward(self, x):
...
tensorplay.save(m.state_dict(), 'mean.pt')

m_loaded = RunningMean(4)
m_loaded.load_state_dict(tensorplay.load('mean.pt'))
assert(tensorplay.all(m.mean == m_loaded.mean))
```

As mentioned previously, buffers can be left out of the module's `state_dict` by marking them as non-persistent:

```{code-block} python
self.register_buffer('unserialized_thing', tensorplay.randn(5), persistent=False)
```
...
m.to(device='cuda', dtype=tensorplay.float64)
```

Buffers of a module can be iterated over using {func}`~tensorplay.nn.Module.buffers` or
{func}`~tensorplay.nn.Module.named_buffers`.
...
class StatefulModule(nn.Module):
  def __init__(self):
    ...
    self.param1 = nn.Parameter(tensorplay.randn(2))

    # Alternative string-based way to register a parameter.
    self.register_parameter('param2', nn.Parameter(tensorplay.randn(3)))
    ...
    self.param_list = nn.ParameterList([nn.Parameter(tensorplay.randn(2)) for i in range(3)])

    # Registers a dictionary of parameters.
    self.param_dict = nn.ParameterDict({
      'foo': nn.Parameter(tensorplay.randn(3)),
      'bar': nn.Parameter(tensorplay.randn(4))
    })

    # Registers a persistent buffer (one that appears in the module's state_dict).
    self.register_buffer('buffer1', tensorplay.randn(4), persistent=True)

    # Registers a non-persistent buffer (one that does not appear in the module's state_dict).
    self.register_buffer('buffer2', tensorplay.randn(5), persistent=False)
...
tensorplay.save(m.state_dict(), 'state.pt')
m_loaded = StatefulModule()
m_loaded.load_state_dict(tensorplay.load('state.pt'))
...
By default, parameters and floating-point buffers for modules provided by {mod}`tensorplay.nn` are initialized during
...
```{code-block} python
m = nn.BatchNorm2d(3, dtype=tensorplay.half)
print(m.running_mean)
: tensor([0., 0., 0.], dtype=tensorplay.float16)
```

While module writers can use any device or dtype to initialize parameters in their custom modules, good practice is
to use `dtype=tensorplay.float` and `device='cpu'` by default as well. Optionally, you can provide full flexibility
in these areas for your custom module by conforming to the convention demonstrated above that all
{mod}`tensorplay.nn` modules follow:
...
- Provide a `dtype` constructor kwarg that applies to any parameters / floating-point buffers registered by
  the module.
- Only use initialization functions (i.e. functions from {mod}`tensorplay.nn.init`) on parameters and buffers within the
  module's constructor. Note that this is only required to use {func}`~tensorplay.nn.utils.skip_init`; see
  [this page](https://www.tensorplay.cn/docs/docs/stable/generated/tensorplay.nn.utils.skip_init.html) for an explanation.
...
over this process, TensorPlay provides "hooks" that can perform arbitrary computation during a forward or backward
...
you haven't written yourself, meaning this functionality can be applied to third-party or TensorPlay-provided modules.

TensorPlay provides two types of hooks for modules:

- **Forward hooks** are called during the forward pass. They can be installed for a given module with
  {func}`~tensorplay.nn.Module.register_forward_pre_hook` and {func}`~tensorplay.nn.Module.register_forward_hook`.
  ...
  {func}`~tensorplay.nn.modules.module.register_module_forward_pre_hook` and
  {func}`~tensorplay.nn.modules.module.register_module_forward_hook` functions.
- **Backward hooks** are called during the backward pass. They can be installed with
  {func}`~tensorplay.nn.Module.register_full_backward_pre_hook` and {func}`~tensorplay.nn.Module.register_full_backward_hook`.
  These hooks will be called when the backward for this Module has been computed.
  {func}`~tensorplay.nn.Module.register_full_backward_pre_hook` will allow the user to access the gradients for outputs
  while {func}`~tensorplay.nn.Module.register_full_backward_hook` will allow the user to access the gradients
  both the inputs and outputs. Alternatively, they can be installed globally for all modules with
  {func}`~tensorplay.nn.modules.module.register_module_full_backward_hook` and
  {func}`~tensorplay.nn.modules.module.register_module_full_backward_pre_hook`.
...
def backward_hook(m, grad_inputs, grad_outputs):
  ...
  new_grad_inputs = [tensorplay.ones_like(gi) * 42. for gi in grad_inputs]
  return new_grad_inputs

# Create sample module & input.
m = nn.Linear(3, 3)
x = tensorplay.randn(2, 3, requires_grad=True)
...
TensorPlay also provides several more advanced features that are designed to work with modules. All these functionalities
